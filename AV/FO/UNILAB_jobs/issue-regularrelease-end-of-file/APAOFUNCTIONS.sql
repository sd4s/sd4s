...
BEGIN
 ...
 lvi_ret_code := UNAPISC.CREATESAMPLE( avs_st,
   ...
   THEN
     ...
     APAOGEN.LogError(lcs_function_name, 'CreateSample-FAILED: sc=' || avs_sc || ',st=' || avs_st || ',cic=' || avs_create_ic || ',cpg=' || avs_create_pg || ',mr=' || avs_modify_reason);
     RAISE APAOGEN.API_FAILED;
   ELSE
     APAOGEN.LogError(lcs_function_name, 'CreateSample-SUCCEEDED: sc=' || avs_sc || ',st=' || avs_st || ',cic=' || avs_create_ic || ',cpg=' || avs_create_pg || ',mr=' || avs_modify_reason);
   END IF;
