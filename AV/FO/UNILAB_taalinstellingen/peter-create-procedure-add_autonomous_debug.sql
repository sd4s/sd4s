--create new DEBUG-procedure with AUTONOMOUS-TRANSACTION (as copy of procedure: ADD_DEBUG)
--
create or replace PROCEDURE add_autonomous_debug
...
as
--add autonomous transaction for debugging...
PRAGMA AUTONOMOUS_TRANSACTION;
--
l_message clob;
l_type varchar2(3);
--
procedure add (p_description in varchar2
              ,p_msg in clob)
is
begin
  if p_msg is not null
  then l_message := l_message||p_description||chr(10)||p_msg||chr(10);
  end if;
end;
--
begin
...
exception
  ...
  then
    ...
    insert into atdebug
      (dbg_message, dbg_type, dbg_table, sc, pg, pgnode, pa, panode, me, menode, ss_from, ss_to)
    values
      (l_message, 'EXC', p_table, p_sc, p_pg, p_pgnode, p_pa, p_panode, p_me, p_menode, p_ss_from, p_ss_to);
    --
    commit;
    --	
end;
