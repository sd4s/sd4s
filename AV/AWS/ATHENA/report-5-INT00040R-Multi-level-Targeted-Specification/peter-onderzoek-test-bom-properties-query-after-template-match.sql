...
/*
WITH tplColumnOrder AS 
(SELECT	header_id
,	MIN(tpl_start_pos) AS tpl_start_pos
FROM template
GROUP BY header_id
)
, BoMProperties AS 
(SELECT	p.part_no
,       p.revision
,       p.item_number, p.branch                              --&PARTS_EXTRA_FIELDS
,	    t.tpl_part_no
,       t.tpl_revision
,	    t.hierarchy, t.functiongrp, t.function_description   --&TPL_EXTRA_FIELDS
...
,   pl.format_id
,	tco.tpl_start_pos
,	t.tpl_field_relevance_bit
,	CASE	WHEN pl.field_id = 27  AND UPPER(h.description) = 'PROPERTY' THEN 0
			WHEN pl.field_id = 5 THEN 0	ELSE 1
	END AS field_relevance_bit
FROM templateParts       p
JOIN template            t ON (   t.tpl_part_no  = p.tpl_part_no 
                              AND t.tpl_revision = p.tpl_revision 
							  AND t.functiongrp  = p.functiongrp 
							  AND t.function_description = p.function_description  
							  )
JOIN tplColumnOrder    tco ON (tco.header_id = t.header_id)
JOIN specification_prop sp ON (	  sp.part_no			= p.part_no
...
JOIN property_layout pl ON (   pl.layout_id		= ss.display_format
                           ...
                           AND (						   
		   (pl.field_id = 1	 AND sp.num_1	IS NOT NULL			AND t.num_1 IS NOT NULL)
	    OR (pl.field_id = 2	 AND sp.num_2	IS NOT NULL			AND t.num_2 IS NOT NULL)
	    OR (pl.field_id = 3	 AND sp.num_3	IS NOT NULL			AND t.num_3 IS NOT NULL)
	    OR (pl.field_id = 4	 AND sp.num_4	IS NOT NULL			AND t.num_4 IS NOT NULL)
	    OR (pl.field_id = 5	 AND sp.num_5	IS NOT NULL			AND t.num_5 IS NOT NULL)
	    OR (pl.field_id = 6	 AND sp.num_6	IS NOT NULL			AND t.num_6 IS NOT NULL)
	    OR (pl.field_id = 7  AND sp.num_7	IS NOT NULL			AND t.num_7 IS NOT NULL)
	    OR (pl.field_id = 8	 AND sp.num_8	IS NOT NULL			AND t.num_8 IS NOT NULL)
	    OR (pl.field_id = 9  AND sp.num_9	IS NOT NULL			AND t.num_9 IS NOT NULL)
	    OR (pl.field_id = 10 AND sp.num_10	IS NOT NULL			AND t.num_10 IS NOT NULL)
	    OR (pl.field_id = 11 AND sp.char_1	IS NOT NULL			AND t.char_1 IS NOT NULL)
	    OR (pl.field_id = 12 AND sp.char_2	IS NOT NULL			AND t.char_2 IS NOT NULL)
	    OR (pl.field_id = 13 AND sp.char_3	IS NOT NULL			AND t.char_3 IS NOT NULL)
	    OR (pl.field_id = 14 AND sp.char_4	IS NOT NULL			AND t.char_4 IS NOT NULL)
	    OR (pl.field_id = 15 AND sp.char_5	IS NOT NULL			AND t.char_5 IS NOT NULL)
	    OR (pl.field_id = 16 AND sp.char_6	IS NOT NULL			AND t.char_6 IS NOT NULL)
	    OR (pl.field_id = 17 AND sp.boolean_1 <> 'N'				AND t.boolean_1 IS NOT NULL)
	    OR (pl.field_id = 18 AND sp.boolean_2 <> 'N'				AND t.boolean_2 IS NOT NULL)
	    OR (pl.field_id = 19 AND sp.boolean_3 <> 'N'				AND t.boolean_3 IS NOT NULL)
	    OR (pl.field_id = 20 AND sp.boolean_4 <> 'N'				AND t.boolean_4 IS NOT NULL)
	    OR (pl.field_id = 21 AND sp.date_1	IS NOT NULL			AND t.date_1 IS NOT NULL)
	    OR (pl.field_id = 22 AND sp.date_2	IS NOT NULL			AND t.date_2 IS NOT NULL)
	    OR (pl.field_id = 23 AND sp.uom_id	IS NOT NULL			AND t.tpl_uom_id IS NOT NULL)
	    OR (pl.field_id = 32 AND sp.test_method	<> 0			AND t.tpl_test_method IS NOT NULL)
	    OR (pl.field_id = 26 AND sp.characteristic	IS NOT NULL	AND t.characteristic IS NOT NULL)
	    OR (pl.field_id = 30 AND sp.ch_2	IS NOT NULL				AND t.ch_2 IS NOT NULL)
	    OR (pl.field_id = 31 AND sp.ch_3	IS NOT NULL				AND t.ch_3 IS NOT NULL)
		OR (pl.field_id = 27)
	    )
    )
...
,        t.tpl_revision
,	t.hierarchy, t.functiongrp, t.function_description   --&TPL_EXTRA_FIELDS
,	ss.section_sequence_no, ss.ref_id, ss.type
,	ss.sequence_no
,	sp.section_id, sp.section_rev
,	sp.sub_section_id, sp.sub_section_rev
,	sp.property_group, sp.property, sp.sequence_no
,	sp.num_1 AS sp_num_1
,	sp.num_2 AS sp_num_2
,	sp.num_3 AS sp_num_3
,	sp.num_4 AS sp_num_4
...
,	sp.ch_3
,	pl.layout_id
,	pl.field_id, pl.header_id, pl.start_pos, pl.format_id
,	tco.tpl_start_pos
,	t.tpl_field_relevance_bit
,	CASE	WHEN pl.field_id = 27 AND UPPER(h.description) = 'PROPERTY' THEN 0
			WHEN pl.field_id = 23 THEN 0
                                  ELSE 1
	END
)
SELECT bp.*
...
/*
--ORIGINELE CODE
-DEFAULTH &PARTS_EXTRA_FIELDS	= ',	p.item_number, p.branch';
-DEFAULTH &TPL_EXTRA_FIELDS		= '	,	t.hierarchy, t.functiongrp, t.function_description';
-DEFAULTH &TPL_JOIN_CONDITION	= '	AND t.functiongrp = p.functiongrp AND t.function_description = p.function_description';
*/ 
...
(SELECT p.part_no            AS root_part
 ...
 ,	SUM(CASE WHEN bi.uom = 'kg' THEN bi.quantity * 1.0
                                ELSE NULL        END )  OVER ()   as parent_kg
 ,	1                                                             AS lvl
 ...
 ,	CAST(CASE WHEN 'DEFAULT' = 'REFDATE' THEN p.reference_date
			  ...
			  AND bi.component_part LIKE 'X%' THEN p.issued_date
                                              ELSE coalesce(p.obsolescence_date, CURRENT_DATE)
         END AS date
		)                                                          as part_reference_date
 ,	CASE WHEN p.revision = 1 AND hh.revision = 1 
         AND p.reference_date < coalesce(hh.issued_date, hh.planned_effective_date) THEN '1'
                                                                                    ELSE '0'
...
, BoMProperties AS 
(SELECT	p.part_no                     --changed-from-p-to-t
,       p.revision                    --changed-from-p-to-t
,       p.item_number, p.branch       --changed-from-p-to-t                       --&PARTS_EXTRA_FIELDS
,	    t.tpl_part_no
,       t.tpl_revision
,	    t.hierarchy, t.functiongrp, t.function_description   --&TPL_EXTRA_FIELDS
,	    ss.section_sequence_no
...
,	CASE	WHEN pl.field_id = 27  AND UPPER(h.description) = 'PROPERTY' THEN 0
	...
	END AS field_relevance_bit
FROM filtered       p            --FROM templateParts       p
JOIN template            t ON (   t.tpl_part_no          = p.tpl_part_no 
                              AND t.tpl_revision         = p.tpl_revision 
							  AND t.functiongrp          = p.functiongrp 
							  AND t.function_description = p.function_description  
...
JOIN property_layout pl ON (   pl.layout_id		= ss.display_format
                           AND pl.revision		= ss.display_format_rev
                           AND pl.header_id		= t.header_id
                           AND (  (pl.field_id = 1	 AND sp.num_1	IS NOT NULL			AND t.num_1 IS NOT NULL)
                               OR (pl.field_id = 2	 AND sp.num_2	IS NOT NULL			AND t.num_2 IS NOT NULL)
                               OR (pl.field_id = 3	 AND sp.num_3	IS NOT NULL			AND t.num_3 IS NOT NULL)
                               OR (pl.field_id = 4	 AND sp.num_4	IS NOT NULL			AND t.num_4 IS NOT NULL)
                               OR (pl.field_id = 5	 AND sp.num_5	IS NOT NULL			AND t.num_5 IS NOT NULL)
