--* File UNI00020R_partBOM_explosion.fex
...
-* TODO: Use LEFT JOIN from bom_item to specification_header and mark as missing when no suitable revision can be found
...
ENGINE SQLORA SET DEFAULT_CONNECTION &INTERSPEC_DB
...
-*-INCLUDE UNI00020P_WITH_partList_unittest
-*-EXIT
...
-*TABLE FILE SQLOUT
-*PRINT *
-*-*WHERE ALTERNATIVE EQ 1;
-*-*WHERE ROOT_PART EQ 'XEM_B15-2628XN5_03';
-*WHERE ROOT_PART EQ 'XEM_B15-2628XN6_03';
...
DEFINE FILE SQLOUT
	ALT2/I10	= ALTERNATIVE;
	PRF2/I10	= PREFERRED;
 
	INGREDIENT_VOLUME/D12.4 MISSING ON SOME = IF LEAF GT 0 THEN NORMALIZED_VOLUME ELSE MISSING;
END
TABLE FILE SQLOUT
PRINT
	ROOT_REV/I10
...
-* Volume of parent component in parent part
	PARENT_VOLUME/D12.4
-* Volume of component in 1l of part
	COMPONENT_VOLUME/D12.4
-* Volume corrected for parent volume
	VOLUME/D12.4
	NORMALIZED_VOLUME/D12.4
-* Corrected volume of ingredients (raw materials) only
	INGREDIENT_VOLUME/D12.4
 
	QUANTITY_STR/A16
	UOM/A12V
 
	NUM_1/D12.6 NUM_2/D12.6 NUM_3/D12.6 NUM_4/D12.6 NUM_5/D12.6
	CHAR_1/A255V CHAR_2/A255V CHAR_3/A40V CHAR_4/A40V CHAR_5/A40V
	DATE_1 DATE_2
	BOOLEAN_1/P1 BOOLEAN_2/P1 BOOLEAN_3/P1 BOOLEAN_4/P1
	COMPUTE CH_1/P8 MISSING ON SOME = CH_1;
	COMPUTE CH_2/P8 MISSING ON SOME = CH_2;
 
	COMPUTE CHILDPATH/A292V = IF (LAST PATH EQ '') OR (LAST ROOT_PART NE ROOT_PART) OR (LAST ALT2 NE ALT2)
		THEN PATH
		ELSE IF (LAST PATH GT PATH) THEN LAST PATH
		ELSE LAST CHILDPATH;
 
	PARENT_QUANTITY/D12.6
	NORMALIZED_QUANTITY/D12.6
 
	ISSUED_DATE OBSOLETE_DATE
	REFERENCE_DATE
-* Deprecated; only available for legacy applications
	ROOT_REFERENCE_DATE AS EXPLOSION_DATE
 
	LEAF/D3S
 
	PRICE/D12.6 COST_PER_UOM/A3V CURRENCY/A3V COST_PERIOD/A8V COST/D20.6
 
	COMPUTE FUNCTION_SEQ/P5 = IF FUNCTION NE '' AND LAST FUNCTION EQ FUNCTION THEN LAST FUNCTION_SEQ ELSE PARENT_SEQ;
 
	E_CYCLIC/A1
	E_NOPHANTOMREVISION/I3S
 
&BOM_RAW.EVAL LVL/D10 ITEM_NUMBER/D10
 
BY ROOT_PART/A18V
BY ROOT_REV/P4
BY ROOT_PART_REV/A27
BY ROOT_DESCRIPTION/A60V
BY ROOT_STATUS/D4
BY ROOT_STATUS_TYPE/A11V
BY ROOT_ALTERNATIVE/I10
BY ROOT_PREFERRED/I10
BY ROOT_ISSUED_DATE
...
BY PATH/A80
BY PARENT_PART/A18V
BY PARENT_PART_REV/A27V
BY PARENT_STATUS/D4
BY PARENT_STATUS_TYPE/A11V
BY PARENT_DESCRIPTION/A60V
BY PARENT_FUNCTION/A60V
BY PARENT_REV/I10
BY PARENT_SEQ/P5
BY PARENT_BRANCH/A80
BY BRANCH/A80
BY PATHNODE
BY LOWEST 1 E_ISSUEDAFTEREXPLDATE/A1 AS E_ISSUED
BY BREADCRUMBS/A200
 
&BOM_TREE.EVAL BY ITEM_NUMBER/D10
&BOM_TREE.EVAL BY LVL/D10
 
-* Raw materials layout data
&BOM_RAW.EVAL BY PART_NO/A18V
...
-* If we have multiple FUNCTIONs, prefer the one labeled 'Function' over 'Spec.Function'
BY LOWEST 1 KW_LABEL
BY FUNCTION/A60V
 
&BOM_RAW.EVAL WHERE LEAF GT 0;
 
ON TABLE HOLD AS UNI00016H_EXPLODED FORMAT XFOCUS INDEX PART_NO BRANCH
ON TABLE SET ASNAMES ON
ON TABLE SET HOLDLIST PRINTONLY
END
-RUN
 
-*EX CHECK_FILE MASTER=UNI00016H_EXPLODED
-*-RUN
