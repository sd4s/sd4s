...
grant all on  sc_lims_dal.mv_bom_header_property  to usr_rna_readonly1;


-- Materialized-View indexes:
CREATE UNIQUE INDEX bom_header_property_part_alt_uq    ON sc_lims_dal.mv_bom_header_property USING btree (part_no, revision, alternative, plant, bom_usage);
CREATE UNIQUE INDEX bom_header_property_pk             ON sc_lims_dal.mv_bom_header_property USING btree (part_no, revision, plant, alternative, bom_usage);
CREATE INDEX        bom_header_property_properties_idx ON sc_lims_dal.mv_bom_header_property USING gin (properties);


--TEST-QUERY
SELECT count(*) FROM sc_lims_dal.mv_bom_header_property;
--134.253
--after refresh: 134759


/*
select count(*)
from 
(SELECT distinct bh.part_no
,   bh.revision
,   bh.plant
,   bh.alternative
,   bh.preferred
...
,   ls.preferred 
FROM bom_header      bh
JOIN part             p      ON p.part_no = bh.part_no
JOIN itbomlysource   ls      ON ls.source = p.part_source and ls.layout_type = 3
JOIN itbomlyitem     li      ON li.layout_id = ls.layout_id AND li.revision = ls.layout_rev
JOIN header           h      USING (header_id)
WHERE ls.preferred = 1
);
--lt=3: 134.759
...
,   jsonb_object_agg(h.description
            ...
            WHEN 'characteristic_1'::text THEN ( SELECT to_jsonb(ch1.description) AS to_jsonb
                                                 FROM sc_interspec_ens.characteristic ch1
                                                 WHERE ch1.characteristic_id = bi.ch_1
											   )
            WHEN 'characteristic_2'::text THEN ( SELECT to_jsonb(ch2.description) AS to_jsonb
                                                 FROM sc_interspec_ens.characteristic ch2
                                                 WHERE ch2.characteristic_id = bi.ch_2
											   )
            WHEN 'characteristic_2'::text THEN ( SELECT to_jsonb(ch3.description) AS to_jsonb
                                                 FROM sc_interspec_ens.characteristic ch3
                                                 WHERE ch3.characteristic_id = bi.ch_3
											   )
            ELSE NULL::jsonb
        END)                                                     AS properties
...
-- Materialized-View indexes:
CREATE INDEX        bom_item_property_component_idx         ON sc_lims_dal.mv_bom_item_property USING btree (component_part, alternative, part_no, revision, plant);
CREATE UNIQUE INDEX bom_item_property_part_component_alt_uq ON sc_lims_dal.mv_bom_item_property USING btree (part_no, revision, component_part, alternative, plant, item_number, bom_usage);
CREATE UNIQUE INDEX bom_item_property_pk                    ON sc_lims_dal.mv_bom_item_property USING btree (part_no, revision, plant, alternative, bom_usage, item_number);
CREATE INDEX        bom_item_property_properties_idx        ON sc_lims_dal.mv_bom_item_property USING gin (properties);
CREATE UNIQUE INDEX bom_item_property_text_pk               ON sc_lims_dal.mv_bom_item_property USING btree (((part_no)::text), revision, ((plant)::text), alternative, bom_usage, item_number);
...
-- View indexes:
CREATE INDEX        specification_keyword_function_idx ON sc_lims_dal.mv_specification_keyword USING gin (functionkw);
CREATE INDEX        specification_keyword_keywords_idx ON sc_lims_dal.mv_specification_keyword USING gin (keywords);
CREATE UNIQUE INDEX specification_keyword_uq           ON sc_lims_dal.mv_specification_keyword USING btree (part_no);
...
CREATE INDEX        sc_lims_dal.bom_path_current_destination_part_idx ON sc_lims_dal.mv_bom_path_current USING btree (destination_part_no);
CREATE INDEX        sc_lims_dal.bom_path_current_function_path_idx    ON sc_lims_dal.mv_bom_path_current USING gist (function_path);
CREATE INDEX        sc_lims_dal.bom_path_current_part_path_idx        ON sc_lims_dal.mv_bom_path_current USING gist (part_path);
CREATE UNIQUE INDEX sc_lims_dal.bom_path_current_uq                   ON sc_lims_dal.mv_bom_path_current USING btree (source_part_no, source_revision, hierarchy);
...
-- View indexes:
CREATE INDEX        sc_lims_dal.specification_property_cell_seq_idx ON sc_lims_dal.mv_specification_property USING gin (cell_seq);
CREATE INDEX        sc_lims_dal.specification_property_cells_idx    ON sc_lims_dal.mv_specification_property USING gin (cells);
CREATE UNIQUE INDEX sc_lims_dal.specification_property_part_no_uq   ON sc_lims_dal.mv_specification_property USING btree (part_no, revision, section_id, sub_section_id, property_group, property, section_sequence_no, display_format, attribute);
CREATE INDEX        sc_lims_dal.specification_property_section_idx  ON sc_lims_dal.mv_specification_property USING btree (section_id, part_no, revision, property_group, property, display_format, sequence_no, section_sequence_no, attribute);
...
-- View indexes:
CREATE INDEX        sc_lims_dal.specification_class_idx                 ON sc_lims_dal.mv_specification USING btree (class3_id, part_no, revision);
CREATE INDEX        sc_lims_dal.specification_frame_function_status_idx ON sc_lims_dal.mv_specification USING btree (frame_id, functionkw, status, part_no, revision);
CREATE INDEX        sc_lims_dal.specification_function_idx              ON sc_lims_dal.mv_specification USING btree (functionkw, part_no, revision);
CREATE INDEX        sc_lims_dal.specification_keywords_idx              ON sc_lims_dal.mv_specification USING gin (keywords);
CREATE INDEX        sc_lims_dal.specification_properties_idx            ON sc_lims_dal.mv_specification USING gin (properties);
CREATE INDEX        sc_lims_dal.specification_qrphase_phase_idx         ON sc_lims_dal.mv_specification USING btree (jsonb_object_field_text(jsonb_object_field(jsonb_object_field(jsonb_object_field(jsonb_object_field(properties, 'SAP information'::text), '(none)'::text), 'SAP information'::text), 'QR phase'::text), 'Value'::text), part_no, revision);
CREATE INDEX        sc_lims_dal.specification_qrphase_phase_pidx        ON sc_lims_dal.mv_specification USING btree (jsonb_object_field_text(jsonb_object_field(jsonb_object_field(jsonb_object_field(jsonb_object_field(properties, 'SAP information'::text), '(none)'::text), 'SAP information'::text), 'QR phase'::text), 'Value'::text), part_no, revision) WHERE (jsonb_object_field_text(jsonb_object_field(jsonb_object_field(jsonb_object_field(jsonb_object_field(properties, 'SAP information'::text), '(none)'::text), 'SAP information'::text), 'QR phase'::text), 'Value'::text) IS NOT NULL);
CREATE UNIQUE INDEX sc_lims_dal.specification_uq                        ON sc_lims_dal.mv_specification USING btree (part_no, revision);
...
-- View indexes:
CREATE INDEX        sc_lims_dal.specification_section_display_format_display_format_rev_type_id ON sc_lims_dal.mv_specification_section USING btree (display_format, display_format_rev) INCLUDE (type);
CREATE INDEX        sc_lims_dal.specification_section_part_no_revision_idx                      ON sc_lims_dal.mv_specification_section USING btree (part_no, revision) INCLUDE (type, ref_id);
CREATE INDEX        sc_lims_dal.specification_section_section_id_part_no_rev_idx                ON sc_lims_dal.mv_specification_section USING btree (section_id, part_no, revision);
CREATE UNIQUE INDEX sc_lims_dal.specification_section_uq                                        ON sc_lims_dal.mv_specification_section USING btree (part_no, revision, section_id, sub_section_id, type, ref_id);
