...
-- Cursors
--------------------------------------------------------------------------------
--Selecteer RQ bij TEST-WS
--SELECT a.requestcode, b.subprogramid FROM utwsgkrequestcode a, utwsgksubprogramid b WHERE a.ws = avs_ws AND a.ws = b.ws;
CURSOR lvq_rq_progid(avs_ws_test  IN VARCHAR2) IS
SELECT gkrq.requestcode
,      spi.subprogramid
FROM utwsgkrequestcode   gkrq
,    utwsgksubprogramid  spi
WHERE gkrq.ws = avs_ws_test
AND   gkrq.ws = spi.ws
;
--SELECT op basis van RQ alle aanwezige WS 
--Dit is in ieder geval de PREP-WS, maar aangezien TEST-WS via parallel-proces via normale EVENT-MGR wordt aangemaakt
--kan het ook voorkomen dat de TEST-WS al bestaat.
...
BEGIN
    ...
    LogError(lcs_function_name, ' (WS: ' || avs_ws || ') HandleCreateSubsamples Start.', 3);
    lvn_cnt_subprogid:=0;
    lvn_cnt_ws:=0;
    lvn_cnt:=0;
    lvn_subsc_rows:=0;
	...
	-- Bijv. RQ=22.019.IBO	subproogramid=Y2
    ----------------------------------------------------------------------------
	...
	LOOP --loop 1.1
        ...
        FOR lvr_worksheets_in_rq IN lvq_worksheets_in_rq(lvr_rq_progid.requestcode, lvr_rq_progid.subprogramid) 
		LOOP --loop 1.2
            lvd_loop1_2:=SYSDATE;
			...
			LOOP --loop 1.3 NOTE: lvr_worksheets_in_rq.ws contains the Outdoor-worksheet, avs_ws is the current prep-ws.
                ...
                --
				---------------------------------------------------------------------------------
				--Loop door samples/partno om per positie een subsample aan te gaan maken als copy van sample.
				---------------------------------------------------------------------------------
	...
	LOOP --loop 2.1        lvd_loop1_1:=SYSDATE;
		...
		LOOP --loop 2.2
				  ...
				  */
                  ...
                  IF lvn_subsc_exists_in_ws=0 THEN
                    ...
                    lvi_ret_code:=UNAPIRQ.SAVE1RQSAMPLE(
                      ...
                      --IF lvi_ret_code=UNAPIGEN.DBERR_SUCCESS THEN
					  begin
  					    LogError (lcs_function_name, 'NA SAVE1RQSAMPLE voor UPDATE SC.RQ voor subsample-id: ' || lvs_sub_sc_id||' dbeer-success: '||UNAPIGEN.DBERR_SUCCESS||' rq: '|| lts_rq_tab(lvn_subsc_row) );
					  ...
					  exception
					    when others 
						then null;
    					     LogError (lcs_function_name, 'EXCP-UPDATE-ERR NA SAVE1RQSAMPLE UPDATE SC.RQ voor subsample-id: ' || lvs_sub_sc_id||sqlerrm);
 					  end;
                    --END IF;
            ...
            IF lvn_sc_already_assigned=0 THEN
              ...
              IF lvi_ret_code != UNAPIGEN.DBERR_SUCCESS THEN
                LogError (lcs_function_name, ' AssignMethod failed for sc <' || lvs_sub_sc_id ||' ME: '||lts_sub_me_tab(lvn_subsc_row) ||'> Function returned :' || lvi_ret_code, 2);
                --lvi_ret_code := UNAPIGEN.SynchrEndTransaction;
              END IF;
