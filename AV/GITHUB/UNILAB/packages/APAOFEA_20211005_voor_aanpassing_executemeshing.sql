CREATE OR REPLACE PACKAGE APAOFEA AS
--------------------------------------------------------------------------------
...
--         WHERE ME.me = 'SX000A'
	...
	CURSOR lvq_me_SX100A IS
		SELECT me.*, imp.importid
          FROM utscme           me
...
--         WHERE me = 'SX000A'
        ...
        IF lvn_total > 0 THEN
            UNAPIGEN.LogError (lcs_function_name, 'There are ' || lvn_total || ', methods of SX000_ In Execution, so we do not start ExecuteMeshing');
        ELSE
		...
		IF lvn_total > 0 THEN
			FOR lvr_me IN lvq_me_SX100A LOOP
                -------------------------------------------------
                -- Determine the equipment, for the correct Lab
                -------------------------------------------------
                lvs_eq := NULL;
                ...
                IF lvs_eq IS NOT NULL THEN
                    ...
                    IF lvs_dir IS NOT NULL THEN
                        ...
                        IF lvs_error IS NOT NULL THEN
                            lvi_ret_code := UNAPIMEP.ADDSCMECOMMENT (lvr_me.sc, lvr_me.pg, lvr_me.pgnode, lvr_me.pa, lvr_me.panode, lvr_me.me, lvr_me.menode
                                    , lvs_error);

                            lvi_ret_code := UNAPIMEP.CHANGESCMESTATUS(
                                    lvr_me.sc, lvr_me.pg, lvr_me.pgnode, lvr_me.pa, lvr_me.panode, lvr_me.me, lvr_me.menode, lvr_me.reanalysis
