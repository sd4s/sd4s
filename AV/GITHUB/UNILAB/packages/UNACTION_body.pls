...
-- 31-01-2022  | PS       | Aanroep UNAPIGEN.LOGERRORNOREMOTEAPICALL uit RQ_A08 gehaald. Procedure bestaat niet in ORACLEPROD !!!
-- 08-11-2022  | PS       | Modify function RQ_A09 (exclude PA with status=@C,CM,TV for updating to AV)
--
--------------------------------------------------------------------------------
...
IS
--Function to update PA.STATUS to AV for all PA within a SC.
--PA already with status = @C (cancelled), CM (Completed) or TV (to-validate) remain there current status.
...
AND a.sc = b.sc
AND b.ss not in ('@C','CM','TV')    --neem PA met status=cancelled/completed/to-validate niet mee in het wijzigen naar ss=AV.
;
BEGIN
  FOR lvr_scpa IN lvq_scpa 
  LOOP
    --change status of PA to AV
...
FUNCTION SC_A09 
RETURN APAOGEN.RETURN_TYPE IS
--Functie om op moment dat FEA-SAMPLE (ST LIKE 'S-T%FEM%') = COMPLETED deze naar betrokken REQUEST-medewerker te mailen.
--FUNCTIE WORDT VANUIT LC AANGEROEPEN INDIEN LC=S1 DE STATUS VERANDERD NAAR COMPLETE !!!
--OP DIT MOMENT ZIJN ALLE SC-PK-ATTRIBUTEN IN UNAPIEV GEVULD...
--
--Er wordt alleen gechecked op errors op SAMPLE-LEVEL voor de sample-types like 'S-T: FEM%' 
--Let op: Zet SYSTEM-SETTING "LOG_INFO" in UTSYSTEM op waarde=1, daarmee krijgen we LOG-INFO in de UTERROR !!!
--        update utsystem SET SETTING_VALUE=1 WHERE SETTING_NAME='LOG_INFO'  and setting_value=0;
--
--unapiev.p_sc   varchar2(100) := 'MVE1910089T11' ;
--unapiev.p_st   varchar2(20) := 'S-T: FEM tyre';
--
--ics_package_name='UNACTION'
--unaction.lcs_function_name   varchar2(100) := 'test-peter-SC-A09' ;
--
lcs_function_name CONSTANT APAOGEN.API_NAME_TYPE := ics_package_name||'.'||'SC_A09';
lvs_sqlerrm        APAOGEN.ERROR_MSG_TYPE;
lvi_ret_code       APAOGEN.RETURN_TYPE;
lvs_value            VARCHAR2(255);
lvi_count            NUMBER;
--
l_unapiev_p_sc       varchar2(100) := null;
l_unapiev_p_st       varchar2(100) := null;
...
BEGIN
  ...
  begin
    l_unapiev_p_sc    := UNAPIEV.P_SC;  --'PSC2127000T01';  -- UNAPIEV.P_SC;
    l_unapiev_p_st    := UNAPIEV.P_ST;  --'S-T: FEM tyre';  -- UNAPIEV.P_ST;
  exception
    when others
    then null;
  end;
  ...
  --avs_message 
  APAOFUNCTIONS.LogInfo(avs_function_name=>lcs_function_name
                       ,avs_message=> 'Mail FEA sample-completed for ' ||'#sc=' || l_unapiev_p_sc ||'#st=' || l_unapiev_p_st
					   );
  --
  if l_unapiev_p_sc is not null
  and l_unapiev_p_st is null
  then
    begin
      select sc.st
      into l_unapiev_p_st
      from utsc sc
      where sc.sc = l_unapiev_p_sc
      ;
    exception
      when others
      then null;
  ...
  --  
  if l_unapiev_p_st is not null
  and l_unapiev_p_st in ('S-T: FEM tyre' ,'S-T: FEM OHT')
  then
    ...
    begin      
	  ...
	  l_recipient_admin_user := 'peter.schepens@Apollotyres.com';
	  l_recipients := l_recipient_sc_user ||';'||l_recipient_admin_user;
	  --Vul evt. subject aan:
	  l_subject := 'Simulations complete for sample: '||l_unapiev_p_sc  ||'-'||l_unapiev_p_st;
	  --VUL eerste regel van email: chr(13)=CarriageRetrun
	  l_index := 1; 
	  l_buffer(l_index) := l_unapiev_p_sc ||chr(13);
	  --VUL tweede regel van email: chr(13)=CarriageRetrun
	  l_index := 2; 
	  l_buffer(l_index) := l_unapiev_p_st||chr(13);
	  --
	  ...
	  --
	  IF lvi_ret_code <> UNAPIGEN.DBERR_SUCCESS 
	  THEN APAOFUNCTIONS.LogInfo(avs_function_name=>lcs_function_name
								,avs_message=> 'FEA-ERROR-SendMail faild to inform error SC <'||l_unapiev_p_sc||'['||l_unapiev_p_st||']>  to recipients <'||l_recipients||'>. Error: '||lvi_ret_code 
								);
	  ELSE l_mail_ok_teller := l_mail_ok_teller + 1;
		   APAOFUNCTIONS.LogInfo(avs_function_name=>lcs_function_name
								,avs_message=> 'FEA-OK-SendMail SUCCESSFULL SC <'||l_unapiev_p_sc||'['||l_unapiev_p_st||']>  to: ' ||l_recipients);
	  END IF;
  ELSE
    --SAMPLE-TYPE is not a FEA-sample-type. No email has to be send.
    APAOFUNCTIONS.LogInfo(avs_function_name=>lcs_function_name
                         ,avs_message=> 'SAMPLE IS NOT FEA, NO further action necessary SC <'||l_unapiev_p_sc||'['||l_unapiev_p_st||']> ' );
  END IF;
  --
  RETURN UNAPIGEN.DBERR_SUCCESS;
EXCEPTION
