...
-- 12-10-2021 | PS        | Extra debug-info voor procedure ME_C03. 
--------------------------------------------------------------------------------
...
BEGIN
   ...
   IF APAOCONDITION.RQORSCPLANNED 
   THEN
      lvi_ret_code := UNAPIGEN.DBERR_SUCCESS;
	  APAOFUNCTIONS.LogInfo(avs_function_name=>lcs_function_name
                         ,avs_message=> 'PG_C02 RQORSCPLANNED: for SC <'||UNAPIEV.P_SC||'> RETURN=DBERR_SUCCESS='||lvi_ret_code ); 
   ELSE
      lvi_ret_code := UNAPIGEN.DBERR_GENFAIL;
	  APAOFUNCTIONS.LogInfo(avs_function_name=>lcs_function_name
                         ,avs_message=> 'PG_C02 RQORSCPLANNED: for SC <'||UNAPIEV.P_SC||'> RETURN=DBERR_GENFAIL='||lvi_ret_code ); 
   END IF;
...
BEGIN
  ...
  --------------------------------------------------------------------------------
  IF NOT APAOCONDITION.RQORSCPLANNED 
  THEN
    lvi_ret_code := UNAPIGEN.DBERR_SUCCESS;
    APAOFUNCTIONS.LogInfo(avs_function_name=>lcs_function_name
                         ,avs_message=> 'PG_C03 NOT-RQORSCPLANNED: for SC <'||UNAPIEV.P_SC||'> RETURN=DBERR_SUCCESS='||lvi_ret_code ); 
   ELSE
    lvi_ret_code := UNAPIGEN.DBERR_GENFAIL;
    APAOFUNCTIONS.LogInfo(avs_function_name=>lcs_function_name
                         ,avs_message=> 'PG_C03 NOT-RQORSCPLANNED: for SC <'||UNAPIEV.P_SC||'> RETURN=DBERR_GENFAIL='||lvi_ret_code ); 
  END IF;
...
BEGIN
   ...
   THEN
     lvi_ret_code :=  UNAPIGEN.DBERR_SUCCESS;
     APAOFUNCTIONS.LogInfo(avs_function_name=>lcs_function_name
                          ,avs_message=> 'PA_C02 RQORSCPLANNED: for SC <'||UNAPIEV.P_SC||'> RETURN=DBERR_SUCCESS='||lvi_ret_code ); 
   ELSE
     lvi_ret_code :=  UNAPIGEN.DBERR_GENFAIL;
     APAOFUNCTIONS.LogInfo(avs_function_name=>lcs_function_name
                          ,avs_message=> 'PA_C02 RQORSCPLANNED: for SC <'||UNAPIEV.P_SC||'> RETURN=DBERR_GENFAIL='||lvi_ret_code ); 
   END IF;
...
BEGIN
  ...
  THEN
    lvi_ret_code := UNAPIGEN.DBERR_SUCCESS;
    APAOFUNCTIONS.LogInfo(avs_function_name=>lcs_function_name
                          ,avs_message=> 'PA_C03 NOT-RQORSCPLANNED: for SC <'||UNAPIEV.P_SC||'> RETURN=DBERR_SUCCESS='||lvi_ret_code ); 
  ELSE	
    lvi_ret_code := UNAPIGEN.DBERR_GENFAIL;
    APAOFUNCTIONS.LogInfo(avs_function_name=>lcs_function_name
                          ,avs_message=> 'PA_C03 NOT-RQORSCPLANNED: for SC <'||UNAPIEV.P_SC||'> RETURN=DBERR_GENFAIL='||lvi_ret_code ); 
  END IF;
...
BEGIN
   ...
   THEN
     lvi_ret_code := UNAPIGEN.DBERR_SUCCESS;
     APAOFUNCTIONS.LogInfo(avs_function_name=>lcs_function_name
                          ,avs_message=> 'ME_C01 RQORSCPLANNED: for SC <'||UNAPIEV.P_SC||'> RETURN=DBERR_SUCCESS='||lvi_ret_code ); 
   ELSE
     lvi_ret_code := UNAPIGEN.DBERR_GENFAIL;
     APAOFUNCTIONS.LogInfo(avs_function_name=>lcs_function_name
                          ,avs_message=> 'ME_C01 RQORSCPLANNED: for SC <'||UNAPIEV.P_SC||'> RETURN=DBERR_GENFAIL='||lvi_ret_code ); 
   END IF;
...
BEGIN
  ...
  --------------------------------------------------------------------------------
   IF NOT APAOCONDITION.RQORSCPLANNED THEN
     ...
     --------------------------------------------------------------------------------
     IF NOT APAOCONDITION.MEWACHTOPANDEREME 
	 THEN
        lvi_ret_code := UNAPIGEN.DBERR_SUCCESS;
        APAOFUNCTIONS.LogInfo(avs_function_name=>lcs_function_name
                             ,avs_message=> 'ME_C02 NOT-RQORSCPLANNED=true + NOT-MEWACHTOPANDEREME=true: for SC <'||UNAPIEV.P_SC||'> RETURN=DBERR_SUCCESS='||lvi_ret_code ); 
     ELSE
        lvi_ret_code := UNAPIGEN.DBERR_GENFAIL;
        APAOFUNCTIONS.LogInfo(avs_function_name=>lcs_function_name
                             ,avs_message=> 'ME_C02 NOT-RQORSCPLANNED=true + NOT-MEWACHTOPANDEREME=false: for SC <'||UNAPIEV.P_SC||'> RETURN=DBERR_GENFAIL='||lvi_ret_code ); 
     END IF;
  ELSE
    lvi_ret_code := UNAPIGEN.DBERR_GENFAIL;
    APAOFUNCTIONS.LogInfo(avs_function_name=>lcs_function_name
                         ,avs_message=> 'ME_C02 NOT-RQORSCPLANNED=true: for SC <'||UNAPIEV.P_SC||'> RETURN=DBERR_GENFAIL='||lvi_ret_code ); 
  END IF;
...
BEGIN
  --
  ----------------------------------------------------------------------------------------------------------------
  -- Alleen indien het opdracht/monster niet de status "Planned" heeft (dus RQORSCPLANNED=FALSE, en dus RQ/SC<>@P)
  -- dan moet waarde SUCCESS=0 worden geretourneerd. 
  -- Want dat is de situatie waarbij een METHOD WEL naar status=WAIT mag, anders moet ME in status @P blijven zitten.  
  -- De constructie <NOT "FALSE"> = TRUE, en gaan we via THEN-tak en retourneren we dus success=0.
  -- --
  -- Indien RQ of SC wel nog status = @P heeft, (dus RQORSCPLANNED=TRUE), dan mag status niet veranderen naar WAIT.
  -- De constructie NOT "TRUE" = FALSE, dus gaan we via ELSE-tak en retourneren we GENFAIL=1.  
  ----------------------------------------------------------------------------------------------------------------
  IF NOT APAOCONDITION.RQORSCPLANNED 
  THEN
      --------------------------------------------------------------------------------
     ...
     lvi_ret_code := UNAPIGEN.DBERR_SUCCESS;
     APAOFUNCTIONS.LogInfo(avs_function_name=>lcs_function_name
                         ,avs_message=> 'ME_C03 NOT-RQORSCPLANNED: for SC <'||UNAPIEV.P_SC||'> + ME <'||UNAPIEV.P_ME||' RETURN=DBERR_SUCCESS='||lvi_ret_code ); 
   ELSE
    lvi_ret_code := UNAPIGEN.DBERR_GENFAIL;
     APAOFUNCTIONS.LogInfo(avs_function_name=>lcs_function_name
                         ,avs_message=> 'ME_C03 NOT-RQORSCPLANNED: for SC <'||UNAPIEV.P_SC||'> + ME <'||UNAPIEV.P_ME||' RETURN=DBERR_GENFAIL='||lvi_ret_code ); 
  END IF;
...
BEGIN
   lvi_ret_code := ME_C03;
   APAOFUNCTIONS.LogInfo(avs_function_name=>lcs_function_name
                        ,avs_message=> 'ME_C04 via ME_C03 NOT-RQORSCPLANNED: for SC <'||UNAPIEV.P_SC||'> RETURN=DBERR_SUCCESS='||lvi_ret_code ); 
   --RETURN(ME_C03);
   RETURN lvi_ret_code;
   --
EXCEPTION
