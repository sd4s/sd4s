...
BEGIN
   ...
   DELETE FROM UAUTCYAU
   WHERE CY = A_CY
   AND VERSION = A_VERSION;

   DELETE FROM UAUTCYHS
   WHERE CY = A_CY
   AND VERSION = A_VERSION;

   DELETE FROM UAUTCYHSDETAILS
   WHERE CY = A_CY
   AND VERSION = A_VERSION;

   RETURN(UNAPIGEN.DBERR_SUCCESS);
END REMOVECYFROMARCHIVE;




FUNCTION COPYCYTOARCHDB(A_CY IN VARCHAR2, A_VERSION IN VARCHAR2, A_IGNORE_DUP_VAL_ON_INDEX BOOLEAN) RETURN NUMBER IS
BEGIN
   
   EXECUTE IMMEDIATE
   'INSERT INTO uautcy' ||
   UNAPIRA.LISTALLCOLUMNS('utcy', 'BRACKETS', '1') ||
   ' SELECT '||   UNAPIRA.LISTALLCOLUMNS('utcy', 'NO_BRACKETS', '1') ||
   ' FROM utcy' ||
   ' WHERE cy = '''||A_CY||'''' ||
   ' AND version = '''||A_VERSION||'''';
   INSERT INTO UAUTCYAU
      (CY, VERSION, AU, AU_VERSION, AUSEQ, VALUE)
   SELECT        CY, VERSION, AU, AU_VERSION, AUSEQ, VALUE
   FROM UTCYAU
   WHERE CY = A_CY
   AND VERSION = A_VERSION;
   INSERT INTO UAUTCYHS
      (CY, VERSION, WHO, WHAT, LOGDATE, LOGDATE_TZ, WHY, WHO_DESCRIPTION,
      WHAT_DESCRIPTION, TR_SEQ, EV_SEQ)
   SELECT  CY, VERSION, WHO, WHAT, LOGDATE, LOGDATE_TZ, WHY, WHO_DESCRIPTION,
      WHAT_DESCRIPTION, TR_SEQ, EV_SEQ
   FROM UTCYHS
   WHERE CY = A_CY
   AND VERSION = A_VERSION;
   INSERT INTO UAUTCYHSDETAILS
      (CY, VERSION, TR_SEQ, EV_SEQ, SEQ, DETAILS)
...
WHEN DUP_VAL_ON_INDEX THEN
   IF A_IGNORE_DUP_VAL_ON_INDEX THEN
      L_RET_CODE := REMOVECYFROMARCHIVE(A_CY, A_VERSION);
   END IF;
...
RETURN NUMBER IS
BEGIN
   L_RET_CODE := COPYCYTOARCHDB(A_CY, A_VERSION, TRUE);
   IF L_RET_CODE = UNAPIGEN.DBERR_NORECORDS THEN
      L_RET_CODE := COPYCYTOARCHDB(A_CY, A_VERSION, FALSE);
      IF L_RET_CODE <> UNAPIGEN.DBERR_SUCCESS THEN
         L_SQLERRM := 'CopyCyToArchDB#return='||TO_CHAR(L_RET_CODE)||' for cy='||A_CY||'#version='||A_VERSION;
         RAISE STPERROR;
      END IF;
   END IF;
...
WHEN OTHERS THEN
   ...
   INSERT INTO UTERROR(CLIENT_ID, APPLIC, WHO, LOGDATE, LOGDATE_TZ, API_NAME, ERROR_MSG)
   VALUES(UNAPIGEN.P_CLIENT_ID, UNAPIGEN.P_APPLIC_NAME, UNAPIGEN.P_USER, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP,
          'ArchiveCyToDB', L_SQLERRM);
   UNAPIGEN.U4COMMIT;
   RETURN(UNAPIGEN.DBERR_GENFAIL);
END ARCHIVECYTODB;

FUNCTION COPYCYFROMARCHDB(A_CY IN VARCHAR2, A_VERSION IN VARCHAR2, A_IGNORE_DUP_VAL_ON_INDEX BOOLEAN) RETURN NUMBER IS
BEGIN
   ...
   INSERT INTO UTCYHSDETAILS
      (CY, VERSION, TR_SEQ, EV_SEQ, SEQ, DETAILS)
   SELECT        CY, VERSION, TR_SEQ, EV_SEQ, SEQ, DETAILS
   FROM UAUTCYHSDETAILS
   WHERE CY = A_CY
   AND VERSION = A_VERSION;
...
WHEN DUP_VAL_ON_INDEX THEN
   IF A_IGNORE_DUP_VAL_ON_INDEX THEN
      L_RET_CODE := REMOVECYFROMDB(A_CY, A_VERSION);
   END IF;
   RETURN(UNAPIGEN.DBERR_NORECORDS);
END COPYCYFROMARCHDB;

FUNCTION RESTORECYFROMDB
(A_CY IN VARCHAR2, A_VERSION IN VARCHAR2)
RETURN NUMBER IS
BEGIN
   L_RET_CODE := COPYCYFROMARCHDB(A_CY, A_VERSION, TRUE);
   IF L_RET_CODE = UNAPIGEN.DBERR_NORECORDS THEN
      L_RET_CODE := COPYCYFROMARCHDB(A_CY, A_VERSION, FALSE);
      IF L_RET_CODE <> UNAPIGEN.DBERR_SUCCESS THEN
         L_SQLERRM := 'CopyCyFromArchDB#return='||TO_CHAR(L_RET_CODE)||' for cy='||A_CY||'#version='||A_VERSION;
         RAISE STPERROR;
      END IF;
   END IF;
...
CURSOR L_UTCY_CURSOR (A_CY IN VARCHAR2, A_VERSION IN VARCHAR2) IS
   SELECT * FROM UDCY WHERE CY=A_CY AND VERSION=A_VERSION;

CURSOR L_UTCYAU_CURSOR (A_CY IN VARCHAR2, A_VERSION IN VARCHAR2) IS
   SELECT * FROM UTCYAU WHERE CY=A_CY AND VERSION=A_VERSION;

CURSOR L_UTCYHS_CURSOR (A_CY IN VARCHAR2, A_VERSION IN VARCHAR2) IS
   SELECT * FROM UTCYHS WHERE CY=A_CY AND VERSION=A_VERSION;

CURSOR L_UTCYHSDETAILS_CURSOR (A_CY IN VARCHAR2, A_VERSION IN VARCHAR2) IS
   SELECT * FROM UTCYHSDETAILS WHERE CY=A_CY AND VERSION=A_VERSION;

BEGIN
   L_SQLERRM:=NULL;
   UNAPIRA.L_EXCEPTION_STEP :='utcy' ||'cy='||A_CY||'#version='||A_VERSION;
   FOR L_REC IN L_UTCY_CURSOR(A_CY, A_VERSION) LOOP
      UNAPIRA3.L_PUTTEXT := 'utcy' || L_SEP ||
      L_REC.CY || L_SEP || L_REC.VERSION || L_SEP ||
      ...
      L_REC.IS_TEMPLATE || L_SEP || L_REC.CHART_TITLE || L_SEP ||
      L_REC.X_AXIS_TITLE || L_SEP || L_REC.Y_AXIS_TITLE || L_SEP ||
      L_REC.Y_AXIS_UNIT || L_SEP || L_REC.X_LABEL || L_SEP ||
      L_REC.DATAPOINT_CNT || L_SEP || L_REC.DATAPOINT_UNIT || L_SEP ||
      L_REC.XR_MEASUREMENTS || L_SEP || L_REC.XR_MAX_CHARTS || L_SEP ||
      ...
      L_REC.CH_LC || L_SEP || L_REC.CH_LC_VERSION || L_SEP ||
      L_REC.INHERIT_AU || L_SEP || L_REC.LAST_COMMENT || L_SEP ||
      L_REC.CY_CLASS || L_SEP || L_REC.LOG_HS || L_SEP ||
   ...
   UNAPIRA.L_EXCEPTION_STEP :='utcyau' ||'cy='||A_CY||'#version='||A_VERSION;
   FOR L_REC IN L_UTCYAU_CURSOR(A_CY, A_VERSION) LOOP
      UNAPIRA3.L_PUTTEXT := 'utcyau' || L_SEP ||
      L_REC.CY || L_SEP || L_REC.VERSION || L_SEP || L_REC.AU || L_SEP ||
      L_REC.AU_VERSION || L_SEP || L_REC.AUSEQ || L_SEP || L_REC.VALUE;
      UNAPIRA3.U4DATAPUTLINE(UNAPIRA3.L_PUTTEXT);
   END LOOP;

   UNAPIRA.L_EXCEPTION_STEP :='utcyhs' ||'cy='||A_CY||'#version='||A_VERSION;
   FOR L_REC IN L_UTCYHS_CURSOR(A_CY, A_VERSION) LOOP
      UNAPIRA3.L_PUTTEXT := 'utcyhs' || L_SEP ||
      L_REC.CY || L_SEP || L_REC.VERSION || L_SEP || L_REC.WHO || L_SEP ||
      L_REC.WHAT || L_SEP ||
      TO_CHAR(L_REC.LOGDATE,UNAPIRA.P_TSTZ_FORMAT) || L_SEP ||
      TO_CHAR(L_REC.LOGDATE_TZ,UNAPIRA.P_TSTZ_FORMAT) || L_SEP ||
      L_REC.WHY || L_SEP || L_REC.WHO_DESCRIPTION || L_SEP ||
      L_REC.WHAT_DESCRIPTION || L_SEP || L_REC.TR_SEQ || L_SEP ||
      L_REC.EV_SEQ;
      UNAPIRA3.U4DATAPUTLINE(UNAPIRA3.L_PUTTEXT);
   END LOOP;

   UNAPIRA.L_EXCEPTION_STEP :='utcyhsdetails' ||'cy='||A_CY||'#version='||A_VERSION;
   FOR L_REC IN L_UTCYHSDETAILS_CURSOR(A_CY, A_VERSION) LOOP
      UNAPIRA3.L_PUTTEXT := 'utcyhsdetails' || L_SEP ||
      L_REC.CY || L_SEP || L_REC.VERSION || L_SEP || L_REC.TR_SEQ || L_SEP ||
...
WHEN UTL_FILE.INVALID_PATH THEN
   ...
   UNAPIRA.UTLFILEEXCEPTIONHANDLER('ArchiveCyToFile', L_SQLERRM, 'UTL_FILE.INVALID_PATH',UNAPIRA.P_CLOSE_CURSOR);
   RETURN(UNAPIGEN.DBERR_NOOBJECT);

WHEN UTL_FILE.INVALID_MODE THEN
   L_SQLERRM := 'directory='||UNAPIRA.P_FILE_DIR||'#file='||UNAPIRA.P_FILE_NAME ||':Invalid mode';
   UNAPIRA.UTLFILEEXCEPTIONHANDLER('ArchiveCyToFile', L_SQLERRM, 'UTL_FILE.INVALID_MODE',UNAPIRA.P_CLOSE_CURSOR);
   RETURN(UNAPIGEN.DBERR_GENFAIL);

WHEN UTL_FILE.INVALID_FILEHANDLE THEN
   L_SQLERRM := 'directory='||UNAPIRA.P_FILE_DIR||'#file='||UNAPIRA.P_FILE_NAME ||':Invalid filehandle';
   UNAPIRA.UTLFILEEXCEPTIONHANDLER('ArchiveCyToFile', L_SQLERRM, 'UTL_FILE.INVALID_FILEHANDLE',UNAPIRA.P_CLOSE_CURSOR);
   RETURN(UNAPIGEN.DBERR_GENFAIL);

WHEN UTL_FILE.INVALID_OPERATION THEN
   
   L_SQLERRM := 'directory='||UNAPIRA.P_FILE_DIR||'#file='||UNAPIRA.P_FILE_NAME ||':Invalid operation';
   UNAPIRA.UTLFILEEXCEPTIONHANDLER('ArchiveCyToFile', L_SQLERRM, 'UTL_FILE.INVALID_OPERATION',UNAPIRA.P_CLOSE_CURSOR);
   RETURN(UNAPIGEN.DBERR_NOOBJECT);

WHEN UTL_FILE.READ_ERROR THEN
   L_SQLERRM := 'directory='||UNAPIRA.P_FILE_DIR||'#file='||UNAPIRA.P_FILE_NAME ||':Read error';
   UNAPIRA.UTLFILEEXCEPTIONHANDLER('ArchiveCyToFile', L_SQLERRM, 'UTL_FILE.READ_ERROR',UNAPIRA.P_CLOSE_CURSOR);
   RETURN(UNAPIGEN.DBERR_GENFAIL);

WHEN UTL_FILE.WRITE_ERROR THEN
   L_SQLERRM := 'directory='||UNAPIRA.P_FILE_DIR||'#file='||UNAPIRA.P_FILE_NAME ||':Write error';
   UNAPIRA.UTLFILEEXCEPTIONHANDLER('ArchiveCyToFile', L_SQLERRM, 'UTL_FILE.WRITE_ERROR',UNAPIRA.P_CLOSE_CURSOR);
   RETURN(UNAPIGEN.DBERR_GENFAIL);

WHEN UTL_FILE.INTERNAL_ERROR THEN
   L_SQLERRM := 'directory='||UNAPIRA.P_FILE_DIR||'#file='||UNAPIRA.P_FILE_NAME ||':Internal error';
   UNAPIRA.UTLFILEEXCEPTIONHANDLER('ArchiveCyToFile', L_SQLERRM, 'UTL_FILE.INTERNAL_ERROR',UNAPIRA.P_CLOSE_CURSOR);
   RETURN(UNAPIGEN.DBERR_GENFAIL);
