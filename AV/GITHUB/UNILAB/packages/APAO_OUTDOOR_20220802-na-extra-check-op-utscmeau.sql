	...
	LOOP --loop 2.1        lvd_loop1_1:=SYSDATE;
		...
		LOOP --loop 2.2
			...
			begin
	          ...
	          ;
              LogError (lcs_function_name, 'UTSCME-ALREADY-ASSIGNED voor APAOFUNCTIONS.ASSIGN-METHOD SUB-SC: '||lvs_sub_sc_id||' SUB-ME: '||lts_sub_me_tab(lvn_subsc_row)||' Assigned: '||lvn_sc_already_assigned||' menode: '||lvn_menode);
            exception
			  when others 
			  then
            ...
            IF lvn_sc_already_assigned=0 THEN
			  ...
			  THEN
                LogError (lcs_function_name, ' AssignMethod failed for sub-sc <' || lvs_sub_sc_id ||' sub-ME: '||lts_sub_me_tab(lvn_subsc_row) ||'> Function returned :' || lvi_ret_code ); 
                --lvi_ret_code := UNAPIGEN.SynchrEndTransaction;
              END IF;
			...
			begin
			  ...
			  ;
              LogError (lcs_function_name, 'UTSCME-ALREADY-ASSIGNED na APAOFUNCTIONS.ASSIGN-METHOD voor SUB-SC: '||lvs_sub_sc_id||' SUB-ME: '||lts_sub_me_tab(lvn_subsc_row)||' Assigned: '||lvn_sc_already_assigned||' menode: '||lvn_menode);
            exception
			  when others 
			  then
