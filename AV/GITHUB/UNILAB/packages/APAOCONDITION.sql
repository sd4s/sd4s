...
-- 21-09-2021 | PS        | Altered function RqOrScPlanned, sc-planned incl ss-rejected
--------------------------------------------------------------------------------
...
FUNCTION RqOrScPlanned
RETURN BOOLEAN IS
--Wordt aangeroepen vanuit package UNCONDITION.ME_C03 (en indirect vanuit ME_C04)
...
BEGIN
  --------------------------------------------------------------------------------
  -- Does Sample have 1 a status = (RJ or @P) 
  --------------------------------------------------------------------------------
   SELECT SUM (DECODE (sc.ss,'RJ', 1
                            ,'@P', 1, 0))     --   ics_ss_planned  ,1))
     INTO lvi_counter1
     FROM utsc sc
    WHERE sc.sc = UNAPIEV.P_SC;

  --------------------------------------------------------------------------------
  -- If request or sample does status before or including P THEN: --> return TRUE
  -- RqPlanned checks on status = DV,SU,RJ,@P,RE (so all statusses before and including @P !!!
  --------------------------------------------------------------------------------
  IF RqPlanned = TRUE OR lvi_counter1 > 0 
  THEN
    APAOFUNCTIONS.LogInfo(avs_function_name=>lcs_function_name
                         ,avs_message=>'RQORSCPLANNED: RQ=TRUE(?) OR SC>0 = '||LVI_COUNTER1||' =PLANNED for SC <'||UNAPIEV.P_SC||'> met ME <'||UNAPIEV.P_ME||'> RETURN=TRUE' );              
    --
    RETURN (TRUE);
   ELSE
     APAOFUNCTIONS.LogInfo(avs_function_name=>lcs_function_name
                         ,avs_message=> 'RQORSCPLANNED: RQ=FALSE(?) OR SC>0 = '||LVI_COUNTER1||' =PLANNED for SC <'||UNAPIEV.P_SC||'> met ME <'||UNAPIEV.P_ME||'> RETURN=FALSE' );              
     RETURN (FALSE);
   END IF;
   --
EXCEPTION
