CREATE OR REPLACE PACKAGE APAO_OUTDOOR AS
--------------------------------------------------------------------------------
...
END APAO_OUTDOOR;
/
CREATE OR REPLACE PACKAGE BODY APAO_OUTDOOR AS
--------------------------------------------------------------------------------
--  PROJECT : Vredestein Enschede
-------------------------------------------------------------------------------
--  PACKAGE : APAO_OUTDOOR
...
-- 06-09-2021 | PS        | HandleCreateSubSamples (aangeroepen vanuit WS_A07) 
--                        | query lvq_subprogid_details aangepast 
--                        | door alleen de actuele (niet-gecancellede) samples 
--                        | te selecteren voor subsamplen.
--                        | Tevens in cursor RQ-ID meegeven tbv performance !
-- 06-09-2021 | PS        | CreateWsPrepOutdoor (aangeroepen vanuit WS_A01) 
--                        | aanroep naar InsertWorksheetEvent eruitgehaald.
--                        | Was dubbel. Komt ook via normale LC-status-change.
-- 28-10-2021 | PS        | Add debug-info in CREATEWS-procedure before and
--                        | after the UNAPIWS.CREATEWORKSHEET-call.
-- 11-10-2021 | PS        | CreateWsPrepOutdoor (aangeroepen vanuit WS_A01) 
--                        | aanroep naar InsertWorksheetEvent ER WEER INGEZET
--                        | Was dubbel, maar is noodzakelijk om toch alle SUBSC aan te maken.
--                        | Er zit nog ergens een fout/bug die dit noodzakelijk maakt.
-- 11-11-2021 | PS        | CreateWsPrepOutdoor. Na aanpassen van sortering nogmaals proberen om te 
--                        | zien of we zonder dubbele aanroep kunnen werken. Lukt nu wel.
-- 11-03-2022 | PS        | Procedure HandleCreateSubSamples aangepast. 
--                        | Alle begin/end-transactions in procedure (daarmee ook 
--                        | tussentijdse COMMITS verwijderd en alleen aan het eind
--                        | waardoor of alles of niets gecommit wordt als er iets misgaat.
-- 01-08-2022 | PS        | Voeg extra debugging toe aan HandleCreateSubSamples bij insert UTWSME !!
--------------------------------------------------------------------------------
...
BEGIN
  ...
  END LOOP;
  APAOFUNCTIONS.LogInfo(lcs_function_name, 'WS_A01:' || lcs_function_name || ' Before UNAPIWS.CREATEWORKSHEET for ' ||
                                           '#WT=' || lvs_wt ||
                                           '#WT_VERSION=' || lvs_wt_version ||
                                           '#WS=' || lvs_ws );
  --                                             
  lvi_ret_code := UNAPIWS.CREATEWORKSHEET
                   (lvs_wt,
                    ...
                    lvs_modify_reason);
  --                    
  APAOFUNCTIONS.LogInfo(lcs_function_name, 'WS_A01:' || lcs_function_name || ' After UNAPIWS.CREATEWORKSHEET for ' ||
                                           '#WT=' || lvs_wt ||
                                           '#WT_VERSION=' || lvs_wt_version ||
                                           '#WS=' || lvs_ws );
    ...
    BEGIN
      ...
      IF NVL(lvr_wt.known_ws,'n/a')='n/a' THEN
         lvs_ws        := GetCodemask4Ws(avs_rq);
          -- Create worksheet
          APAOFUNCTIONS.LogInfo(lcs_function_name, 'WS_A01:' || lcs_function_name || ' Before CREATEWS for ' ||
                                           '#WT=' || lvr_wt.wt ||
                                           '#WS=' || lvs_ws ||
                                           '#PR=' || lvr_wt.pr );
          --                                           
          ...
          --
          APAOFUNCTIONS.LogInfo(lcs_function_name, 'WS_A01:' || lcs_function_name || ' After CREATEWS for ' ||
                                           '#WT=' || lvr_wt.wt ||
                                           '#WS=' || lvs_ws ||
                                           '#PR=' || lvr_wt.pr );
          --
...
BEGIN
    ...
    FOR lvr_ws IN lvq_ws_codemask LOOP
       ...
       */
       ...
       --*********************************************************************************
       --PS: DD. 25-08-2021 TIJDELIJK UITGEZET OM AUTOMATISCH METEEN SUBSAMPLES AAN TE MAKEN.
       --    WE LATEN LC-PROCES ZIJN WERK DOEN, EN VANUIT LC=W2 DE SUBSAMPLES AANMAKEN, ZODAT 
       --    WE ZEKER WETEN DAN PREP-WS ONDERTUSSEN DOOR NORMALE EVENT-MGR IS AANGEMAAKT...
	   --PS: DD 11-10-2021 WEER AANGEZET. IS TOCH NOODZAKELIJK VOOR GOEDE WERKING
	   --PS: DD 11-11-2021 TIJDELIJK UITGEZET. NA AANPASSING SORTERING+EXTRA-COMMIT CONTROLEREN
	   --    OF DAT EFFECT HEEFT GEHAD.
...
-- Parameter: avs_ws            = new empty PREP-WS for new created subsamples...
--                                Bijv. 22.049.PSC-W  (=RQ||'-'||SUBPROGRAM-ID)
--            avs_modify_reason = 
-- Creates subsamples, based on existing supersamples already linked to TEST-WS.
--------------------------------------------------------------------------------
RETURN INTEGER IS
--------------------------------------------------------------------------------
-- Constants
...
--------------------------------------------------------------------------------
--Variables
--lvd = local variable date
--lvi = local variable integer
--lvn = local variable number
--lvs = local variable string
--lts = local table string
--------------------------------------------------------------------------------
...
lvs_rq_fallback          VARCHAR2(20); --bewaren RQ voor subsamples in geval er iets niet goed gaat met bewaren van RQ in loop.
--
...
-- Cursors
--------------------------------------------------------------------------------
--Selecteer RQ bij PREP-WS
--SELECT a.requestcode, b.subprogramid FROM utwsgkrequestcode a, utwsgksubprogramid b WHERE a.ws = avs_ws AND a.ws = b.ws;
CURSOR lvq_rq_progid(avs_prep_ws  IN VARCHAR2) IS
SELECT gkrq.requestcode
,      spi.subprogramid
FROM utwsgkrequestcode   gkrq
,    utwsgksubprogramid  spi
WHERE gkrq.ws = avs_prep_ws
AND   gkrq.ws = spi.ws
;
--SELECT op basis van RQ alle aanwezige WS 
--Dit is in ieder geval 1x TEST-WS, maar als er meerdere testen zijn aangevraagd (bijv. remmen/sneeuw + handling) kunnen er meerdere TEST-WS voorkomen.
--En aangezien PREP-WS via parallel-proces via normale EVENT-MGR wordt aangemaakt kan het ook voorkomen dat de PREP-WS al bestaat.
--SELECT a.ws FROM utwsgksubprogramid a,utwsgkrequestcode b WHERE b.requestcode = avs_requestcode AND a.subprogramid = avs_subprogramid AND a.ws = b.ws;
CURSOR lvq_worksheets_in_rq( avs_prep_ws       IN VARCHAR2
                            ,avs_requestcode   IN VARCHAR2
                            ,avs_subprogramid  IN VARCHAR2) IS
SELECT spi.ws
,      gkrq.requestcode
FROM utwsgksubprogramid    spi
,    utwsgkrequestcode     gkrq
WHERE gkrq.requestcode = avs_requestcode
AND   spi.subprogramid = avs_subprogramid
AND   spi.ws           <> avs_prep_ws      --SELECTEER altijd alleen de TEST-WS, ook als je procedure nog een keer draait nadat er al subsamples zijn aangemaakt...
AND   spi.ws           = gkrq.ws
;
--
...
--SELECT a.avtestmethod,b.numberofrefs,c.refsetdesc,d.sc,e.testsetposition,subprogram.SubProgramId,TO_NUMBER(NVL(f.NumberOfVariants,'1')) NumberOfVariants,COUNT(DISTINCT sub_method.SC) scs_already_created
--FROM utwsgkavtestmethod a,utwsgknumberofrefs b, utwsgkrefsetdesc c,utwssc d,utwsgktestsetposition e,utwsgkNumberOfVariants f,utwsgkSubProgramId subprogram,utscmeau sub_method
--WHERE a.ws = avs_odws AND b.ws(+) = a.ws  AND c.ws(+) = a.ws AND d.ws = a.ws AND e.ws(+) = a.ws  AND f.ws(+) = a.ws AND subprogram.ws(+) = a.ws --to identify number of different subprograms
--AND sub_method.au(+)='avSuperSampleSc' --to identify sub-sample method  AND sub_method.value(+)=d.sc --to identify sub-sample method
--GROUP BY a.avtestmethod, --each testmethod needs 1..nr_of_variants*(positions) sub-samples  b.numberofrefs, --each nr of refs needs 1..nr_of_variants*(positions) sub-samples
--c.refsetdesc, --each nr of refs needs 1..nr_of_variants*(positions) sub-samples   d.sc, --each super sample needs 1..nr_of_variants*(positions) sub-samples
--e.testsetposition -- to the know positions (may vary by super_sample) for the sub-samples ,subprogram.SubProgramId --each subprogram sample needs 1..nr_of_variants*(positions) sub-samples
--,f.NumberOfVariants --to know the nr_of_variants  ORDER BY scs_already_created DESC;
--
CURSOR lvq_subprogid_details(avs_odws IN VARCHAR2
                            ,avs_rq   IN VARCHAR2 ) IS
SELECT wsc.ws
,      av.avtestmethod
,      nor.numberofrefs
,      rsd.refsetdesc
...
,      tsp.testsetposition  
,      spi.SubProgramId
,      TO_NUMBER(NVL(nov.NumberOfVariants,'1'))   NumberOfVariants
,      (select count(distinct me.sc) from utscmeau me where me.sc like avs_rq||'%' and me.au = 'avSuperSampleSc' and me.value = wsc.sc)    scs_already_created
FROM utwsgknumberofrefs       nor
,      utwsgkrefsetdesc       rsd
,      utwsgktestsetposition  tsp
,      utwsgkNumberOfVariants nov
,      utwsgkSubProgramId     spi
,      utwsgkavtestmethod     av
,      utwssc                 wsc
WHERE wsc.ws   = avs_odws   --'20.760.RHI-001'
AND wsc.sc in (select sc.sc from utsc sc where sc.sc = wsc.sc and sc.ss not in ('@C') )
AND av.ws     = wsc.ws
AND nor.ws(+) = wsc.ws
AND rsd.ws(+) = wsc.ws
AND tsp.ws(+) = wsc.ws
AND nov.ws(+) = wsc.ws
AND spi.ws(+) = wsc.ws 
ORDER BY 1, 9 DESC, 5 ASC
;
--
CURSOR lvq_part_no (avs_sc IN VARCHAR2, avs_subprog_id IN VARCHAR2) IS
SELECT ii.iivalue
,      substr(ii.ic,-2) as position
,      meau_1.SC, meau_1.PG, meau_1.PGNODE, meau_1.PA, meau_1.PANODE, meau_1.ME, meau_1.MENODE, meau_1.AU, meau_1.AU_VERSION, meau_1.AUSEQ, meau_1.VALUE    --meau_1.*
...
;
--select partno uit SC-IC van SUPERsample
...
WHERE SC=avs_sc
 AND IC='avScReqGen'
 AND II='avPartNo';
--select partno van subsc (wordt niet gebruikt!!)
...
--
BEGIN
    --AVS_WS = NIEUW PREP-WS waaronder de aan te maken SUBSAMPLES moeten komen te hangen. Bijv. 22.019.IBO-Y2
    LogError(lcs_function_name, ' (PREP-WS: ' || avs_ws || ') APAO-OUTDOOR.HandleCreateSubsamples Start.', 3);
    lvn_cnt_subprogid:=0;
    lvn_cnt_ws:=0;
    lvn_cnt:=0;
    lvn_subsc_rows:=0;
    ...
    lvs_ws:=avs_ws;
    ----------------------------------------------------------------------------
    -- Retrieve all occurances request/programid for new PREP-worksheet
	-- Bijv. RQ=22.019.IBO	subprogramid=Y2
    ----------------------------------------------------------------------------
    FOR lvr_rq_progid IN lvq_rq_progid(lvs_ws) 
	LOOP --loop 1.1
	     --BIJV: 22.049.PSC	   W
        lvd_loop1_1:=SYSDATE;
        ...
        lvn_cnt:=lvn_cnt+1;
		--Peter: bewaren RQ
        lvs_rq_fallback := lvr_rq_progid.requestcode;
        --
		--Retrieve ALL WS which exists for RQ. For the first time, only TEST-WS, second-time also PREP-WS.
		--Retrieve TEST-WS + PREP-WS op bij deze RQ/subprogramId, Bijv. 22.019.IBO-001 + 22.019.IBO-Y2
        FOR lvr_worksheets_in_rq IN lvq_worksheets_in_rq(lvs_ws, lvr_rq_progid.requestcode, lvr_rq_progid.subprogramid) 
		LOOP --loop 1.2
		     --BIJV.	22.049.PSC-001	22.049.PSC      --bestaande test-ws 1
             --         22.049.PSC-002	22.049.PSC      --bestaande test-ws 2
             --         22.049.PSC-W	22.049.PSC		--NIEUWE prep-ws
            lvd_loop1_2:=SYSDATE;
			--
            LogError (lcs_function_name, ' (TEST-WS: ' || lvr_worksheets_in_rq.ws || ') HandleCreateSubsamples loop1.2 Start.');
            ----------------------------------------------------------------------------
            --  Retrieve detailed information for these TEST-worksheets
            ----------------------------------------------------------------------------
            --PS: uit perf-overwegingen wordt RQ-CODE meegegeven !!!
			--
			--Uitzoeken: Wat zijn de verschillende combinaties van SAMPLE-TYPE (normal/combi/mixed-set) - PARTNO - TESTMETHOD - TESTSETPOSITIONS
			--
			--Uitzoeken: is het mogelijk om LVR_WS_DETAILS alleen uit te voeren voor WS met grootste aantal testsetpositions?
			--           Dan kan dan ook automatisch SUPERSAMPLE worden voor de aan te maken SUBSAMPLES.
			--           Bijv: Als normal-set WS supersamples bevat met een testmethod waarbij max. 4x testsetpositions (FL/FR/RL/RR) horen dan nemen we deze eerst
			--           Of:   Als normal-set WS supersamples bevat met een testmothod waarbij max. 2x testsetpositions (FL/FR) horen dan nemen we deze.
			--           Of:   Als combination-set WS supersamples bevat met een testmethod waarbij max 2x. testsetpositions (FL/FR) horen dan nemen we deze
			--           Of:   Als mixed-set WS supersamples bevat met een testmethod waarbij max 1x testsetposition (FL, FR, RL, RR) is dan nemen we deze 
			--
            FOR lvr_ws_details IN lvq_subprogid_details(lvr_worksheets_in_rq.ws, lvr_rq_progid.requestcode) 
			LOOP --loop 1.3 
			     --NOTE: lvr_worksheets_in_rq.ws contains the Outdoor-worksheet, avs_ws is the current prep-ws.
 			     --Bij eerste keer runnen, komen alleen de gewone samples via TEST-WS voor, en nog niet de subsamples via PREP-WS.
				 --Voor deze TEST-WS-samples moeten we subsamples aan gaan maken...
				--
  			    --BIJV.: Vanuit TEST-WS: 22.019.IBO-001:
			    --TT802AA	1		22.019.IBO02	FL;FR	Y2	2	0
                --TT802AA	1		22.019.IBO01	FL;FR	Y2	2	0
                --OF:
				--ws				method  numofref sc             	testsetposition   subpr	numVar	scs-already-created
                --22.049.PSC-001	TT803AA	1		22.049.PSC01		FL;FR				W		3		4
                --22.049.PSC-001	TT803AA	1		22.049.PSC02		FL;FR				W		3		4
                --22.049.PSC-001	TT803AA	1		22.049.PSC03		FL;FR				W		3		4
				--
                --22.049.PSC-002	TT842AA	1		22.049.PSC01		FL;FR;RL;RR			W		3		4
                --22.049.PSC-002	TT842AA	1		22.049.PSC02		FL;FR;RL;RR			W		3		4
                --22.049.PSC-002	TT842AA	1		22.049.PSC03		FL;FR;RL;RR			W		3		4
                --
                lvd_loop1_3:=SYSDATE;
                --LogError (lcs_function_name, ' (' || avs_ws || ') HandleCreateSubsamples loop1.3 Start.');
                ----------------------------------------------------------------------------
                -- Retrieve all part_no (sample infofields) positions for the samples of the found requests within SUBPROGRAM-ID
                ----------------------------------------------------------------------------
                ...
                --
				---------------------------------------------------------------------------------
				--Loop door samples/partno en per positie een subsample aan te gaan maken als copy van sample.
				--let op: als er meerdere testmethodes aan via assign-full-testplan aan sample zijn aangemaakt
				--        dan levert lvr_part_no dubbele records op (met alleen op pa/me-objecten kleine verschillen) !!!
				---------------------------------------------------------------------------------
                FOR lvr_part_no IN lvq_part_no(lvr_ws_details.sc, lvr_rq_progid.subprogramid) 
				LOOP 
				  --loop 1.4
				  --Indien 2 samples met 2 verschillende banden, maar met dezelfde TESTMETHODE:
				  --XEF_E20B259C	FL	22.019.IBO01	Outdoor testing	1000000	Brake wet ABS dist	1000000	TT802A	2000000	avCustSubSampleST		2	WheelSetEntity
                  --XEF_E20B259C	FR	22.019.IBO01	Outdoor testing	1000000	Brake wet ABS dist	1000000	TT802A	2000000	avCustSubSampleST		2	WheelSetEntity
                  --XEF_E20B259C	RL	22.019.IBO01	Outdoor testing	1000000	Brake wet ABS dist	1000000	TT802A	2000000	avCustSubSampleST		2	WheelSetEntity
                  --XEF_E20B259C	RR	22.019.IBO01	Outdoor testing	1000000	Brake wet ABS dist	1000000	TT802A	2000000	avCustSubSampleST		2	WheelSetEntity
				  --
                  --XEF_E20B259D	FL	22.019.IBO02	Outdoor testing	1000000	Brake wet ABS dist	1000000	TT802A	2000000	avCustSubSampleST		2	WheelSetEntity
                  --XEF_E20B259D	FR	22.019.IBO02	Outdoor testing	1000000	Brake wet ABS dist	1000000	TT802A	2000000	avCustSubSampleST		2	WheelSetEntity
                  --XEF_E20B259D	RL	22.019.IBO02	Outdoor testing	1000000	Brake wet ABS dist	1000000	TT802A	2000000	avCustSubSampleST		2	WheelSetEntity
                  --XEF_E20B259D	RR	22.019.IBO02	Outdoor testing	1000000	Brake wet ABS dist	1000000	TT802A	2000000	avCustSubSampleST		2	WheelSetEntity
				  --
				  --Indien 3 samples met 3 verschillende banden, maar met 2 aanwezige TESTMETHODES (braking 2x FL/FR + handling 4x FL/FR/RL/RR):
				  --IIVALUE			POS	SC				PG				PGNODE	PA					PANODE	ME		MENODE	AU				    AUSEQ	VALUE			avCustSubSampleST		avCustSubSamplePP	avCustSubSamplePR	avCustSubSampleMT	MELINK	    SubProgramID
                  --XEF_E20B259A	FL	22.049.PSC01	Outdoor testing	1000000	Brake snow ABS dist	1000000	TT803A	2000000	avCustSubSampleST		2	WheelSetEntity	WheelSetEntity	0001.08	Mounting			Mounting	0001.09	TP800A1	0001.62		92310005	W
                  --XEF_E20B259A	FL	22.049.PSC01	Outdoor testing	1000000	Handling snow		2000000	TT842A	2000000	avCustSubSampleST		2	WheelSetEntity	WheelSetEntity	0001.08	Mounting			Mounting	0001.09	TP800A1	0001.62		92310006	W
                  --XEF_E20B259A	FR	22.049.PSC01	Outdoor testing	1000000	Brake snow ABS dist	1000000	TT803A	2000000	avCustSubSampleST		2	WheelSetEntity	WheelSetEntity	0001.08	Mounting			Mounting	0001.09	TP800A1	0001.62		92310005	W
                  --XEF_E20B259A	FR	22.049.PSC01	Outdoor testing	1000000	Handling snow		2000000	TT842A	2000000	avCustSubSampleST		2	WheelSetEntity	WheelSetEntity	0001.08	Mounting			Mounting	0001.09	TP800A1	0001.62		92310006	W
                  --XEF_E20B259A	RL	22.049.PSC01	Outdoor testing	1000000	Brake snow ABS dist	1000000	TT803A	2000000	avCustSubSampleST		2	WheelSetEntity	WheelSetEntity	0001.08	Mounting			Mounting	0001.09	TP800A1	0001.62		92310005	W
                  --XEF_E20B259A	RL	22.049.PSC01	Outdoor testing	1000000	Handling snow		2000000	TT842A	2000000	avCustSubSampleST		2	WheelSetEntity	WheelSetEntity	0001.08	Mounting			Mounting	0001.09	TP800A1	0001.62		92310006	W
                  --XEF_E20B259A	RR	22.049.PSC01	Outdoor testing	1000000	Brake snow ABS dist	1000000	TT803A	2000000	avCustSubSampleST		2	WheelSetEntity	WheelSetEntity	0001.08	Mounting			Mounting	0001.09	TP800A1	0001.62		92310005	W
                  --XEF_E20B259A	RR	22.049.PSC01	Outdoor testing	1000000	Handling snow		2000000	TT842A	2000000	avCustSubSampleST		2	WheelSetEntity	WheelSetEntity	0001.08	Mounting			Mounting	0001.09	TP800A1	0001.62		92310006	W
                  --
                  --XEF_E20B259B	FL	22.049.PSC02	Outdoor testing	1000000	Brake snow ABS dist	1000000	TT803A	2000000	avCustSubSampleST		2	WheelSetEntity	WheelSetEntity	0001.08	Mounting			Mounting	0001.09	TP800A1	0001.62		92310007	W
                  --XEF_E20B259B	FL	22.049.PSC02	Outdoor testing	1000000	Handling snow		2000000	TT842A	2000000	avCustSubSampleST		2	WheelSetEntity	WheelSetEntity	0001.08	Mounting			Mounting	0001.09	TP800A1	0001.62		92310008	W
                  --XEF_E20B259B	FR	22.049.PSC02	Outdoor testing	1000000	Brake snow ABS dist	1000000	TT803A	2000000	avCustSubSampleST		2	WheelSetEntity	WheelSetEntity	0001.08	Mounting			Mounting	0001.09	TP800A1	0001.62		92310007	W
                  --XEF_E20B259B	FR	22.049.PSC02	Outdoor testing	1000000	Handling snow		2000000	TT842A	2000000	avCustSubSampleST		2	WheelSetEntity	WheelSetEntity	0001.08	Mounting			Mounting	0001.09	TP800A1	0001.62		92310008	W
                  --XEF_E20B259B	RL	22.049.PSC02	Outdoor testing	1000000	Brake snow ABS dist	1000000	TT803A	2000000	avCustSubSampleST		2	WheelSetEntity	WheelSetEntity	0001.08	Mounting			Mounting	0001.09	TP800A1	0001.62		92310007	W
                  --XEF_E20B259B	RL	22.049.PSC02	Outdoor testing	1000000	Handling snow		2000000	TT842A	2000000	avCustSubSampleST		2	WheelSetEntity	WheelSetEntity	0001.08	Mounting			Mounting	0001.09	TP800A1	0001.62		92310008	W
                  --XEF_E20B259B	RR	22.049.PSC02	Outdoor testing	1000000	Brake snow ABS dist	1000000	TT803A	2000000	avCustSubSampleST		2	WheelSetEntity	WheelSetEntity	0001.08	Mounting			Mounting	0001.09	TP800A1	0001.62		92310007	W
                  --XEF_E20B259B	RR	22.049.PSC02	Outdoor testing	1000000	Handling snow		2000000	TT842A	2000000	avCustSubSampleST		2	WheelSetEntity	WheelSetEntity	0001.08	Mounting			Mounting	0001.09	TP800A1	0001.62		92310008	W
                  --
                  --XEF_E20B259C	FL	22.049.PSC03	Outdoor testing	1000000	Brake snow ABS dist	1000000	TT803A	2000000	avCustSubSampleST		2	WheelSetEntity	WheelSetEntity	0001.08	Mounting			Mounting	0001.09	TP800A1	0001.62		92310009	W
                  --XEF_E20B259C	FL	22.049.PSC03	Outdoor testing	1000000	Handling snow		2000000	TT842A	2000000	avCustSubSampleST		2	WheelSetEntity	WheelSetEntity	0001.08	Mounting			Mounting	0001.09	TP800A1	0001.62		92310010	W
                  --XEF_E20B259C	FR	22.049.PSC03	Outdoor testing	1000000	Brake snow ABS dist	1000000	TT803A	2000000	avCustSubSampleST		2	WheelSetEntity	WheelSetEntity	0001.08	Mounting			Mounting	0001.09	TP800A1	0001.62		92310009	W
                  --XEF_E20B259C	FR	22.049.PSC03	Outdoor testing	1000000	Handling snow		2000000	TT842A	2000000	avCustSubSampleST		2	WheelSetEntity	WheelSetEntity	0001.08	Mounting			Mounting	0001.09	TP800A1	0001.62		92310010	W
                  --XEF_E20B259C	RL	22.049.PSC03	Outdoor testing	1000000	Brake snow ABS dist	1000000	TT803A	2000000	avCustSubSampleST		2	WheelSetEntity	WheelSetEntity	0001.08	Mounting			Mounting	0001.09	TP800A1	0001.62		92310009	W
                  --XEF_E20B259C	RL	22.049.PSC03	Outdoor testing	1000000	Handling snow		2000000	TT842A	2000000	avCustSubSampleST		2	WheelSetEntity	WheelSetEntity	0001.08	Mounting			Mounting	0001.09	TP800A1	0001.62		92310010	W
                  --XEF_E20B259C	RR	22.049.PSC03	Outdoor testing	1000000	Brake snow ABS dist	1000000	TT803A	2000000	avCustSubSampleST		2	WheelSetEntity	WheelSetEntity	0001.08	Mounting			Mounting	0001.09	TP800A1	0001.62		92310009	W
                  --XEF_E20B259C	RR	22.049.PSC03	Outdoor testing	1000000	Handling snow		2000000	TT842A	2000000	avCustSubSampleST		2	WheelSetEntity	WheelSetEntity	0001.08	Mounting			Mounting	0001.09	TP800A1	0001.62		92310010	W
                  --
				  ...
				  --LTS_Position_TAB in (FL/FR/RL/RR) die voorkomen als testsetposition bij WS
                  FOR lvn_position_row IN lts_position_tab.FIRST..lts_position_tab.LAST 
				  LOOP --loop 1.5
				       --Array lts_position_tab bevat combinatie SAMPLE/PARTNO, POSITION en SCME. 
					   --De combinatie SAMPLE/PART-NO + POSITION kan dubbel voorkomen indien meerdere testmethodes zijn aangemaakt per sample.
                    lvd_loop1_5:=SYSDATE;
                    --LogError (lcs_function_name, ' (' || avs_ws || ') HandleCreateSubsamples loop1.5 Start.');
                    --
					--Indien part-no/sample-position in reeks van WS-testsetpositions voorkomt dan SUBSAMPLE maken !!
                    IF lts_position_tab(lvn_position_row) = lvr_part_no.position 
					THEN
                      lvn_subsc_rows:=lvn_subsc_rows+1;
                      --                                                                (10) + . + Y2 + . + 12 + . + FL >>>> 19 chars left=1.
					  --first part to create the sub sample
                      lts_sub_sc_tab(lvn_subsc_rows) :=  lvr_ws_details.sc||'.'|| SUBSTR(lvr_rq_progid.subprogramid,1,2); 
                      --
    ...
    --***************************************************************************
	--Interne ARRAYS zijn gevuld met alle SUPER-sample-info.
	--We gaan nu op basis hiervan de SUBSAMPLES aanmaken !!
	--LVS_WS = NIEUWE PREP-WS VOOR SUBSAMPLES
    --***************************************************************************
    FOR lvn_subsc_row IN 1..lvn_subsc_rows 
	LOOP --loop 2.1        lvd_loop1_1:=SYSDATE;
	     -- In lvn_subsc_rows staan aantal aan te maken SUBSAMPLES...
        lvd_loop2_1:=SYSDATE;
        --LogError (lcs_function_name, ' (' || avs_ws || ') HandleCreateSubsamples loop2.1 Start.');
        ----------------------------------------------------------------------------
        -- Determine the number of samples in the sub program
        ----------------------------------------------------------------------------
        FOR lvn_subsc_cnt IN 1..ltn_set_size_tab(lvn_subsc_rows) 
		LOOP --loop 2.2
            lvd_loop2_2:=SYSDATE;
            ...
            ----------------------------------------------------------------------------
            ----------------------------------------------------------------------------
            -- Determine the unique code of the new SUBsample
            ----------------------------------------------------------------------------
            lvs_sub_sc_id:=lts_sub_sc_tab(lvn_subsc_row)||'.'||SUBSTR(lts_position_id_tab(lvn_subsc_row),1,2);
            LogError (lcs_function_name, ' AOPA_OUTDOOR.CreateSubSamples( PREP-WS: ' || lvs_ws || ') Additional Sub-SC:' ||lvs_sub_sc_id||' loop2.2 ');
			--
            ...
            lvs_super_sc := SUBSTR(lts_super_me_link_tab(lvn_subsc_row),
                            lvn_posnr,INSTR(lts_super_me_link_tab(lvn_subsc_row),'#',lvn_posnr) -lvn_posnr);
            --
            ----------------------------------------------------------------------------
            -- Check if the SUBsample has not been assigned yet to the PREP-worksheet in UTWSSC
            ----------------------------------------------------------------------------
            ...
            OR ( ltn_sub_sc_created_tab (lvn_subsc_row) = 0 )    --sc already-created is al bepaald in ws-detail-cursor voor test-ws!!
            ...
            THEN
               ...
               IF lvn_sc_already_assigned>0 AND lvn_subsc_offset=0 
			   THEN
                 LogError (lcs_function_name, ' AOPA_OUTDOOR.CreateSubSamples(' || lvs_ws || ') WARNING Additional Sub SC:' ||lvs_sub_sc_id);
                 FOR lvn_subsc_offset IN 1..99 
				 LOOP
                   lvs_sub_sc_id:=lts_sub_sc_tab(lvn_subsc_row)||'.'||SUBSTR(lts_position_id_tab(lvn_subsc_row),1,2);
                   --
               ...
               --------------------------------------------------------------------------------------------
               --PS: 2022-03-11: VERWIJDER DE COMMIT WEER OM TE ZIEN OF HET OOK ZONDER KAN...
               --lvi_ret_code := UNAPIGEN.BeginTransaction; --CopySC Transaction
			   --************************************************************************************************************************
			   --************************************************************************************************************************
			   --************************************************************************************************************************
               LogError (lcs_function_name, ' starttime: ' || SYSDATE || ' APAO_OUTDOOR.COPYSC super-SC: '||lvs_super_sc||' SUB-SC=' || lvs_sub_sc_id);
               lvi_ret_code := APAO_OUTDOOR.COPYSC(lvs_super_sc, lts_sub_st_tab(lvn_subsc_row), lvs_sub_sc_id , avs_modify_reason);
			   --
			   UPDATE UTSC SET RQ=lts_rq_tab(lvn_subsc_row) WHERE SC=lvs_sub_sc_id AND RQ is null;
			   --
			   --************************************************************************************************************************
			   --************************************************************************************************************************
			   --************************************************************************************************************************
               ...
               --
               IF (lvi_ret_code != UNAPIGEN.DBERR_SUCCESS) AND (lvi_ret_code != UNAPIGEN.DBERR_SCALREADYEXIST) 
			   THEN
                  LogError (lcs_function_name, ' CopySample failed for SUB-SC <' || lvs_sub_sc_id || '> (<>dberr_success OF scalreadyexist) Function returned :' || lvi_ret_code);
                  lvn_cnt:=lvn_cnt+1;
                  --PS: 2022-03-11: VERWIJDER DE COMMIT WEER OM TE ZIEN OF HET OOK ZONDER KAN...
               ...
               ELSE
				  ...
				  */
				  ...
				  THEN
                    ...
                    lvi_ret_code:=UNAPIRQ.SAVE1RQSAMPLE(lts_rq_tab(lvn_subsc_row)
					                                   ,lvs_sub_sc_id
													   ,CURRENT_TIMESTAMP
													   ,'UNILAB'
													   ,avs_modify_reason);
                      ...
                      --IF lvi_ret_code=UNAPIGEN.DBERR_SUCCESS THEN
					  begin
  					    LogError (lcs_function_name, 'NA SAVE1RQSAMPLE voor UPDATE SC.RQ voor subsample-id: ' || lvs_sub_sc_id||' dberr-success: '||UNAPIGEN.DBERR_SUCCESS||' rq: '|| lts_rq_tab(lvn_subsc_row) );
						if lts_rq_tab(lvn_subsc_row)  is null
						then  UPDATE UTSC SET RQ=lvs_rq_fallback           WHERE SC=lvs_sub_sc_id AND RQ is null;
						else  UPDATE UTSC SET RQ=lts_rq_tab(lvn_subsc_row) WHERE SC=lvs_sub_sc_id AND RQ is null;
						end if;
  					    LogError (lcs_function_name, 'NA SAVE1RQSAMPLE na UPDATE SC.RQ voor subsample-id: ' || lvs_sub_sc_id);
					  exception
					    when others 
						then null;
    					     LogError (lcs_function_name, 'EXCP-OTHERS-UPDATE-ERR NA SAVE1RQSAMPLE UPDATE SC.RQ voor subsample-id: ' || lvs_sub_sc_id||sqlerrm);
 					  end;
                    --END IF;
                  END IF;  --lvn_subsc_exists_in_ws=0
				  --
               END IF;  --(lvi_ret_code != UNAPIGEN.DBERR_SUCCESS) AND (lvi_ret_code != UNAPIGEN.DBERR_SCALREADYEXIST)
			   --
            END IF;  --( lvn_sc_already_assigned = 0 ) OR ( ltn_sub_sc_created_tab (lvn_subsc_row) = 0 )
			--
            --existence of sub-SC in the PREP-WS at this point must be guaranteed
			begin
              SELECT rownr
              INTO lvn_max_ws_pos
              FROM utwssc
              ...
              AND   sc = lvs_sub_sc_id
			  ;
              LogError (lcs_function_name, 'UTWSSC-MAX-ROWNR WS: '||lvs_ws||' SUB-SC: '||lvs_sub_sc_id||' rownr-max-ws-pos: '||lvn_max_ws_pos);
            exception
			  when others 
			  then
                LogError (lcs_function_name, 'EXCP-OTHER-UTWSSC-MAX-ROWNR WS: '||lvs_ws||' SUB-SC: '||lvs_sub_sc_id||' rownr-max-ws-pos: '||lvn_max_ws_pos||sqlerrm);
			end;
            --
            ...
            FOR l_ii IN lc_part_no_ii(lvs_sub_sc_id) 
			LOOP
			...
			--Aanmaken van UTSCPG zit standaard in de COPYSC-procedure!!! 
			--Voor de zekerheid hier nogmaals controleren, maar is meestal niet nodig.
            BEGIN
			  ...
			  ;
              LogError (lcs_function_name, 'UTSCPG-ALREADY-ASSIGNED voor APAOFUNCTIONS.ASSIGN-PARAMETERGROUP SUB-SC: '||lvs_sub_sc_id||' SUB-PG: '||lts_sub_pg_tab(lvn_subsc_row)||' Assigned: '||lvn_sc_already_assigned||' PAnode: '||lvn_pgnode);
            exception
			  when others 
			  then
                LogError (lcs_function_name, 'EXCP-ERROR-UTSCPA-ALREADY-ASSIGNED voor APAOFUNCTIONS.ASSIGN-PARAMETERGROUP voor SUB-SC: '||lvs_sub_sc_id||' SUB-PG: '||lts_sub_PG_tab(lvn_subsc_row)||' Assigned: '||lvn_sc_already_assigned||' menode: '||lvn_pgnode);
			end;
            --
            IF lvn_sc_already_assigned=0 
			THEN
              ...
              lvi_ret_code := APAOFUNCTIONS.AssignParametergroup(lvs_sub_sc_id,
												   ...
												   lvn_pgnode,
                                                   lts_sub_st_tab(lvn_subsc_row),      --NOTE: PP-KEY applies to Vredestein config only
                                                   ' ',
												   ' ',
												   ' ',
              ...
              --LogError (lcs_function_name, ' AssignParametergroup ' || lts_sub_pg_tab(lvn_subsc_row) || ' done for <' || lvs_sub_sc_id || '> the function took from ' || SYSDATE || ' to ' || lvd_sub);
               ...
               END IF;
            END IF;   --lvn_sc_already_assigned=0
            --PS: 2022-03-11: VERWIJDER DE COMMIT WEER OM TE ZIEN OF HET OOK ZONDER KAN...
			...
			--
            --Aanmaken van UTSCPA zit standaard in de COPYSC-procedure!!! 
			--Voor de zekerheid hier nogmaals controleren, maar is meestal niet nodig.
			begin
			  ...
			  ;
              LogError (lcs_function_name, 'UTSCPA-ALREADY-ASSIGNED voor APAOFUNCTIONS.ASSIGN-PARAMETER SUB-SC: '||lvs_sub_sc_id||' SUB-PA: '||lts_sub_pa_tab(lvn_subsc_row)||' Assigned: '||lvn_sc_already_assigned||' PAnode: '||lvn_panode);
            exception
			  when others 
			  then
                LogError (lcs_function_name, 'EXCP-ERROR-UTSCPA-ALREADY-ASSIGNED voor APAOFUNCTIONS.ASSIGN-PARAMETER voor SUB-SC: '||lvs_sub_sc_id||' SUB-PA: '||lts_sub_PA_tab(lvn_subsc_row)||' Assigned: '||lvn_sc_already_assigned||' menode: '||lvn_panode);
			end;
            --
            IF lvn_sc_already_assigned=0 
			THEN
            ...
            END IF;  --lvn_sc_already_assigned=0
            --
			--Aanmaken van UTSCME zit NIET standaard in de COPYSC-procedure!!! 
			--Deze is mandatory en moet altijd worden doorlopen...
			begin
              LogError (lcs_function_name, 'INPUT-QUERY-UTSCME-ALREADY-ASSIGNED SUB-SC: '||lvs_sub_sc_id||' (voorheen-SC-selectie VIA: '||lts_sub_sc_tab(lvn_subsc_row)||') SUB-PG: '||lts_sub_pg_tab(lvn_subsc_row)||' SUB-PA: '||lts_sub_pa_tab(lvn_subsc_row)||' SUB-ME: '||lts_sub_me_tab(lvn_subsc_row) );
              SELECT COUNT(1), MAX(menode)
              INTO lvn_sc_already_assigned, lvn_menode
              FROM UTSCME
              WHERE SC = lts_sub_sc_tab(lvn_subsc_row)    --lvs_sub_sc_id                   --PS-02-08-2022: was voorheen: lts_sub_sc_tab(lvn_subsc_row), GEEN IDEE WAT ERIN STAAT, MAAR IS NIET CONSISTENT MET DE REST !!!!
              AND   PG = lts_sub_pg_tab(lvn_subsc_row)
              AND   PA = lts_sub_pa_tab(lvn_subsc_row)   --bevat waarde = lvr_part_no.avCustSubSamplePR = Mounting
              AND   ME = lts_sub_me_tab(lvn_subsc_row)   --bevat waarde = lvr_part_no.avCustSubSampleMT = TP800A1 
	          ;
              LogError (lcs_function_name, 'UTSCME-ALREADY-ASSIGNED voor APAOFUNCTIONS.ASSIGN-METHOD SUB-SC: '||lvs_sub_sc_id||' (voorheen-SC-selectieVIA: '||lts_sub_sc_tab(lvn_subsc_row)||') SUB-ME: '||lts_sub_me_tab(lvn_subsc_row)||' Assigned: '||lvn_sc_already_assigned||' menode (GEVULD?): '||lvn_menode);
            exception
			  when others 
			  then
                LogError (lcs_function_name, 'EXCP-ERROR-UTSCME-ALREADY-ASSIGNED voor APAOFUNCTIONS.ASSIGN-METHOD voor SUB-SC: '||lvs_sub_sc_id||' SUB-ME: '||lts_sub_me_tab(lvn_subsc_row)||' Assigned: '||lvn_sc_already_assigned||' menode: '||lvn_menode);
			end;
            --
            IF lvn_sc_already_assigned=0 
			THEN
              ...
              lvi_ret_code := APAOFUNCTIONS.AssignMethod(avs_sc=>lvs_sub_sc_id
			                                            ,avs_pg=>lts_sub_pg_tab(lvn_subsc_row)
														,avn_pgnode=>lvn_pgnode
														,avs_pp_version=>lts_sub_ppversion_tab(lvn_subsc_row)
														,avs_pp_key1=>lts_sub_st_tab(lvn_subsc_row) --NOTE: applies to Vredestein config only
														,avs_pp_key2=>' '
														,avs_pp_key3=>' '
														,avs_pp_key4=>' '
														,avs_pp_key5=>' ' --NOTE: applies to Vredestein config only
                                                        ,avs_pa=>lts_sub_pa_tab(lvn_subsc_row)
                                                        ,avn_panode=>lvn_panode
                                                        ,avs_pr_version=>lts_sub_prversion_tab(lvn_subsc_row)
                                                        ,avs_me=>lts_sub_me_tab(lvn_subsc_row)
                                                        ,avn_menode=>lvn_menode
                                                        ,avs_mt_version=>lts_sub_mtversion_tab(lvn_subsc_row)
                                                        ,avn_reanalysis=>lvn_reanalysis
                                                        ,avs_modify_reason=>avs_modify_reason||lts_super_me_link_tab(lvn_subsc_row) --may exceed 255 at very low risk
														,avb_with_details=>TRUE
                                                        );

              --LogError(lcs_function_name, ' AssignMethod ' || lts_sub_me_tab(lvn_subsc_row) || '  for <' || lvs_sub_sc_id || '> the function ended: ' || SYSDATE);
              IF lvi_ret_code = UNAPIGEN.DBERR_SUCCESS 
			  THEN
                LogError (lcs_function_name, 'UTSCME-APAOFUNCTIONS.AssignMethod = OK for sub-sc <' || lvs_sub_sc_id ||' sub-ME: '||lts_sub_me_tab(lvn_subsc_row) ||'> Function returned :' || lvi_ret_code ); 
                --lvi_ret_code := UNAPIGEN.SynchrEndTransaction;
			  ELSE
                LogError (lcs_function_name, 'UTSCME-APAOFUNCTIONS.AssignMethod FAILED for sub-sc <' || lvs_sub_sc_id ||' sub-ME: '||lts_sub_me_tab(lvn_subsc_row) ||'> Function returned :' || lvi_ret_code ); 
              END IF;
            ...
            --
			--controle na de INSERT van UTSCME of alles goed gegaan is...Alleen voor LOGGING interessant.
			begin
              SELECT COUNT(1), MAX(menode)
              INTO lvn_sc_already_assigned, lvn_menode
              FROM UTSCME
			  ...
			  ;
              LogError (lcs_function_name, 'UTSCME-ALREADY-ASSIGNED na APAOFUNCTIONS.ASSIGN-METHOD voor SUB-SC: '||lvs_sub_sc_id||' SUB-ME: '||lts_sub_me_tab(lvn_subsc_row)||' Assigned: '||lvn_sc_already_assigned||' menode (NA INSERT WEL GEVULD?): '||lvn_menode);
            exception
			  when others 
			  then
                LogError (lcs_function_name, 'EXCP-ERROR-UTSCME-ALREADY-ASSIGNED na APAOFUNCTIONS.ASSIGN-METHOD voor SUB-SC: '||lvs_sub_sc_id||' SUB-ME: '||lts_sub_me_tab(lvn_subsc_row)||' Assigned: '||lvn_sc_already_assigned||' menode: '||lvn_menode);
			end;
            --
            IF lvn_menode IS NOT NULL 
			THEN
              --When Me is created or exists, assign the melink (N:1 link).
			  --Doordat we vanuit meerdere SAMPLES kan het zijn dat ME en MEAU reeds veel eerder al zijn aangemaakt.
			  --Dus ook voor MEAU eerst checken of deze al voor SUB-SC zijn aangemaakt.
			  --Alle MEAU worden in 1 array opgenomen en binnen 1 actie aangemaakt
			  begin
                SELECT COUNT(1)
                INTO lvn_sc_already_assigned
                FROM UTSCMEAU
                ...
                AND   ME = lts_sub_me_tab(lvn_subsc_row)
	            ;
                LogError (lcs_function_name, 'UTSCMEAU-ALREADY-ASSIGNED na APAOFUNCTIONS.ASSIGN-METHOD voor SUB-SC: '||lvs_sub_sc_id||' SUB-ME: '||lts_sub_me_tab(lvn_subsc_row)||' Assigned: '||lvn_sc_already_assigned);
              exception
			    when others 
			    then
                  LogError (lcs_function_name, 'EXCP-ERROR-UTSCMEAIU-ALREADY-ASSIGNED na APAOFUNCTIONS.ASSIGN-METHOD voor SUB-SC: '||lvs_sub_sc_id||' SUB-ME: '||lts_sub_me_tab(lvn_subsc_row)||' Assigned: '||lvn_sc_already_assigned||'-'||sqlerrm);
			  end;
			  --SELECT * FROM UTSCMEAU WHERE SC='22.050.PSC01.W.FR' AND ME='TP800A1' 
              --22.050.PSC01.W.FR	Mounting	1000000	Mounting	1000000	TP800A1	1000000	avSubSampleMeLink		1	92310012
              --22.050.PSC01.W.FR	Mounting	1000000	Mounting	1000000	TP800A1	1000000	avSuperSampleSc			2	22.050.PSC01
              --22.050.PSC01.W.FR	Mounting	1000000	Mounting	1000000	TP800A1	1000000	avSuperSamplePg			3	Outdoor testing#1000000
              --22.050.PSC01.W.FR	Mounting	1000000	Mounting	1000000	TP800A1	1000000	avSuperSamplePa			4	Handling snow#2000000
              --22.050.PSC01.W.FR	Mounting	1000000	Mounting	1000000	TP800A1	1000000	avSuperSampleMe			5	TT842A#2000000
			  --22.050.PSC01.W.FR	Mounting	1000000	Mounting	1000000	TP800A1	1000000	avSqlCurrentPartno		6	XEF_E20B259A
              --
			  --Indien nog geen MEAU aanwezig dan toevoegen...
			  IF lvn_sc_already_assigned=0 
			  THEN
                ...
                if lvi_ret_code = UNAPIGEN.DBERR_SUCCESS 
			    THEN
                  LogError (lcs_function_name, ' UNAPIMEP.SAVESCMEATTRIBUTES VIA LTS_VALUE_TAB = OK for SUB-SC <' || lvs_sub_sc_id ||' SUB-ME: '||lts_sub_me_tab(lvn_subsc_row) ||'> Function returned :' || lvi_ret_code );
				ELSE
                  LogError (lcs_function_name, ' UNAPIMEP.SAVESCMEATTRIBUTES VIA LTS_VALUE_TAB failed for SUB-SC <' || lvs_sub_sc_id ||' SUB-ME: '||lts_sub_me_tab(lvn_subsc_row) ||'> Function returned :' || lvi_ret_code );
                  --lvi_ret_code := UNAPIGEN.SynchrEndTransaction;
                END IF;
				--
			  end if; --UTSCMEAU 
			  ...
			  begin
				...
				;
				LogError (lcs_function_name, 'UTWSME-ALREADY-ASSIGNED-VOOR-INSERT, PREP-WS: '||LVS_WS||' SUB-SC: '||lvs_sub_sc_id||' SUB-ME: '||lts_sub_me_tab(lvn_subsc_row)||' Assigned: '||lvn_sc_already_assigned);
              exception
			    when others
				then 
				  lvn_sc_already_assigned := 0;
                  LogError (lcs_function_name, 'EXCP-UTWSME-ERR-ASSIGNED-count, PREP-WS: '||LVS_WS||' SUB-SC: ' || lvs_sub_sc_id||sqlerrm);
              end;
              --
              IF lvn_sc_already_assigned=0 
              THEN
                begin
                  ...
                  --SAVEPOINT UNILAB4;
                  LogError (lcs_function_name, 'NA Insert WSME = OK, PREP-WS: '||LVS_WS||' SUB-SC: '||lvs_sub_sc_id||' SUB-ME: '||lts_sub_me_tab(lvn_subsc_row) );
				  --
                exception
                  when others
                  then null;
                    LogError (lcs_function_name, 'EXCP-INSERT-UTWSME PREP-WS: '||LVS_WS||' SUB-SC: ' || lvs_sub_sc_id||' SUB-ME: '||lts_sub_me_tab(lvn_subsc_row)||sqlerrm);
                end;
				--
              END IF; --UTWSME: lvn-sc-already-assigned=0
			  --
            END IF;  --lvn_menode IS NOT NULL
			--
			--PS: 2021-11-09: EXTRA COMMIT INGEBOUWD OM DE WIJZIGINGEN OP UTWSME OP TE SLAAN 
			--PS: 2022-03-11: VERWIJDER DE COMMIT WEER OM TE ZIEN OF HET OOK ZONDER KAN...
			--PS: 2022-08-02: EXTRA COMMIT WEER AANGEZET...
            lvi_ret_code := UNAPIGEN.SynchrEndTransaction; 
			--
			begin
                SELECT COUNT(1)
                ...
                AND MENODE=lvn_menode
				;
				LogError (lcs_function_name, 'EINDE LOOP2.2: UTWSME-ALREADY-ASSIGNED-NA-INSERT, PREP-WS: '||LVS_WS||' SUB-SC: '||lvs_sub_sc_id||' SUB-ME: '||lts_sub_me_tab(lvn_subsc_row)||' Assigned: '||lvn_sc_already_assigned);
              exception
			    when others
				then 
				  lvn_sc_already_assigned := 0;
                  LogError (lcs_function_name, 'EXCP-UTWSME-ASSIGNED-count EINDE LOOP2.2: NA INSERT, PREP-WS: '||LVS_WS||' SUB-SC: ' || lvs_sub_sc_id||sqlerrm);
              end;
    ...
    LogError (lcs_function_name, ' (EINDE CREATE-SUBSAMPLES PREP-PW: ' || avs_ws || ') HandleCreateSubsamples End. Started at: ' || lvd_start_datetime, 3);
    RETURN UNAPIGEN.DBERR_SUCCESS;
	--
EXCEPTION
WHEN OTHERS THEN
