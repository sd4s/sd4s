create or replace PACKAGE BODY            "AAPIMIGRATEDATA"
AS
   ...
   BEGIN
      ...
      THEN
         ...
         EXCEPTION
            WHEN NO_DATA_FOUND
            THEN
               LNRETVAL := ( IAPIGENERAL.SETERRORTEXTANDLOGINFO( IAPICONSTANTDBERROR.DBERR_NOBOMITEMDISPLAYFORMAT,
                                                           LNDISPLAYFORMATID,
                                                           LNDISPLAYFORMATREVISION,
                                                           ANHEADER ) );
            WHEN TOO_MANY_ROWS
   ...
   BEGIN
      ...
      BEGIN
         ...
         UPDATE SPECIFICATION_HEADER
            SET STATUS = ANSTATUS,
                OBSOLESCENCE_DATE = ADOBSOLESCENCE,
   ...
   EXCEPTION
      WHEN NO_DATA_FOUND
      THEN
         RETURN( IAPIGENERAL.SETERRORTEXT( GSSOURCE,
                                           LSMETHOD,
   ...
   BEGIN
      ...
      LOOP
         BEGIN
            ...
            IF ( LNRETVAL <> IAPICONSTANTDBERROR.DBERR_SUCCESS )
            THEN
               LNRETVAL := IAPIGENERAL.SETERRORTEXTANDLOGINFO(GSSOURCE, LSMETHOD, LNRETVAL);
               IF LNRETVAL <> 352 THEN
                   dbms_output.put_line(LSPARTNO || '[' || LNREVISION || ']: ' || IAPIGENERAL.GETLASTERRORTEXT());
               END IF;
               LNRESULT := LNRETVAL;
            END IF;
         EXCEPTION
   ...
   EXCEPTION
      WHEN OTHERS
      THEN
         IF UTL_FILE.IS_OPEN( LRFILEHANDLE ) THEN
            UTL_FILE.FCLOSE( LRFILEHANDLE );
         END IF;
         IAPIGENERAL.LOGERROR( GSSOURCE,
                               LSMETHOD,
         ...
         BEGIN
            ...
            THEN
               IAPIGENERAL.LOGINFO( GSSOURCE,
                                    ...
                                    IAPIGENERAL.GETLASTERRORTEXT( ) );
                IF LNRETVAL <> 78 THEN
                    dbms_output.put_line(LSPARTNO || '[]: ' || IAPIGENERAL.GETLASTERRORTEXT());
                END IF;
                LNRESULT := LNRETVAL;
            END IF;
         EXCEPTION
   ...
   BEGIN
      ...
      LOOP
         BEGIN
              --dbms_output.put_line('>>> Line ' || lnCount);
            ...
            IF ( LQINFO%ISOPEN ) THEN CLOSE LQINFO; END IF;
            IF ( LQERRORS%ISOPEN ) THEN CLOSE LQERRORS; END IF;
            ...
            IF LNRETVAL = IAPICONSTANTDBERROR.DBERR_ERRORLIST THEN
              ...
              THEN
                 ...
                 LOOP
                    ...
                    IAPIGENERAL.LOGERROR(GSSOURCE, LSMETHOD, LRERROR.ERRORTEXT);
                 END LOOP;
              END IF;
            END IF;
            ...
            THEN
               dbms_output.put_line('>>> Line ' || lnCount);
               dbms_output.put_line(LSPARTNO || '[' || LNREVISION || ']-' || LNSECTION || '-' || LNSUBSECTION || '-' || LNPROPERTYGROUP || '-' || LNPROPERTY || '-' || LNATTRIBUTE ||', ' || LSVALUE || ': ' || IAPIGENERAL.GETLASTERRORTEXT());
               LNRETVAL := IAPIGENERAL.SETERRORTEXTANDLOGINFO(GSSOURCE, LSMETHOD, LNRETVAL);
               LNRESULT := LNRETVAL;
            END IF;
         EXCEPTION
