...
set define off;
  ...
  CREATE OR REPLACE PROCEDURE "INTERSPC"."GETACTUALLICENSEUSAGE" 
AS
   l_ret                        NUMBER;
   ...
   lvd_job_last_run             TIMESTAMP WITH TIME ZONE;
BEGIN
   lvs_step := '0';
   ...
   lvd_job_last_run := CURRENT_TIMESTAMP;
   l_ret :=
        ...
        IF a_logon_date (i) IS NOT NULL THEN
            ...
            BEGIN
                SELECT osuser, SID, machine, terminal,
                         logon_time, CASE program
                                     WHEN 'intercfg.exe' THEN 'Configuration Module'
                                     WHEN 'interfrm.exe' THEN 'Frame Module'
                                     WHEN 'interspc.exe' THEN 'Spec Module'
                                     WHEN 'w3wp.exe' THEN 'Spec Module Web'
                                     ELSE program 
                                     END
                  INTO lvs_osuser, lvn_sid, lvs_machine, lvs_terminal,
                         lvd_session_logon_time_d, lvs_executable
                  FROM v$session
            ...
            lvs_step := '14.'||i;
            BEGIN
                SELECT SUBSTR(FORENAME||'.'||LAST_NAME,1,40)  --different from unilab
                  INTO lvs_user_description
                  FROM application_user
                 WHERE USER_ID = a_user_name (i);
                lvs_step := '15.'||i;
            EXCEPTION
                WHEN OTHERS
                THEN
            ...
            */
            ...
            INSERT INTO atlicensecheck
                            (RECORD, lic_consumed,
                             ...
                             lic_cons_serial_id, lic_cons_shortname,
                  ...
                  VALUES (TO_NUMBER (LPAD (i, 2, '0')), a_lic_consumed (i),
                             ...
                             a_lic_consumed_serial_id(i), a_lic_consumed_shortname(i),
   ...
   THEN
      ...
      END IF;
        ...
        IF lvs_step <> '4.1' AND lvs_sqlerrm <> 'ORA-01403: no data found' THEN
          IAPIGENERAL.LogError('xx', 'getactuallicenseusage', 'Step='||lvs_step||' :'||lvs_sqlerrm); --different from UNILAB
