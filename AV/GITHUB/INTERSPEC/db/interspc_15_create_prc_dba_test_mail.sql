...
BEGIN
   dbms_output.put_line('verstuur mail');
   --
   l_connection := utl_smtp.open_connection('18.208.22.77');
   l_isopen := TRUE;

   utl_smtp.helo(l_connection, '@apollotyres.com');                    --smtp-domain
   utl_smtp.mail(l_connection, 'patrick.goossens@apollotyres.com');    --smtp-sender
   utl_smtp.rcpt(l_connection, 'peter.schepens@apollotyres.com');      --recipient (string gescheiden punt-komma)
   utl_smtp.open_data(l_connection);
        send_header(l_connection, 'From',   'patrick.goossens@apollotyres.com');    --sender
        send_header(l_connection, 'To',     'peter.schepens@apollotyres.com');      --recipient
        send_raw_header(l_connection, 'Subject', 'DBA Test Mail');          --subject
