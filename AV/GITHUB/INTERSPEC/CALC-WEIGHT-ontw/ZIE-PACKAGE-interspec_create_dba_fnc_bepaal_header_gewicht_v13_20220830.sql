...
SET SERVEROUTPUT ON
declare
l_component_part             varchar2(100) := 'GM_MT149';  --'ED_700-5-11';  --EF_T235/65R17WNIXS'   --'XGF_BU11S18N1'; --'EM_764';
l_revision                   number        := null;
l_alternative                number        := 1;   --of 2
...
BEGIN
  ...
  then  
    ...
    dbms_output.put_line('HEADER-MAINPART: '||pl_header_part_no ||' revision: '||pl_header_revision||' alternative: '||pl_header_alternative||' show bom-items J/N: '||pl_show_incl_items_jn );
    dbms_output.put_line('**************************************************************************************************************');
  end if;
