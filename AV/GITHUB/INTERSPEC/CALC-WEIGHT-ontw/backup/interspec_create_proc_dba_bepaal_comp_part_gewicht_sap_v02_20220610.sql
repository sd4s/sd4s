    ...
    BEGIN
      ...
      then 
        dbms_output.put_line('l_mainpart'||';'||'l_mainrevision'||';'||'l_mainplant'||';''l_mainalternative'||';'||'l_mainframeid'||
		                    ';'||'l_part_no'||';'||'l_revision'||';'||'l_plant'||';''l_alternative'||';'||'l_component_part'||';'||'l_componentdescription'||';'||'l_componentrevision'||';'||'l_componentalternative'||';'||'l_componentissueddate'||
							';'||'l_characteristic_id'||';'||'l_functiecode'||';'||'l_header_issued_date'||';'||'l_path'||';'||'l_quantity_path'||';'||'l_bom_quantity_kg' );      
      end if;							
      ...
      open c_bom_items for SELECT bi2.LVL
				...
				,      (select to_char(sh.issued_date,'dd-mm-yyyy hh24:mi:ss') from specification_header sh where sh.part_no=b.component_part and sh.status in (126,127,128)) componentissueddate
      ...
      loop 
		...
		DBA_INSERT_WEIGHT_COMP_PART (p_tech_calculation_date=>l_tech_calculation_date
		                            ,p_datum_verwerking=>to_date(l_componentissueddate,'dd-mm-yyyy hh24:mi:ss')
