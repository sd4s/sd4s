...
begin
  l_component_part_eenheid_kg := DBA_BEPAAL_BOM_HEADER_GEWICHT(p_header_part_no=>l_component_part, p_show_incl_items_jn=>'N');
...
--LET OP: ALLE TUSSENLIGGENDE BOM-ITEM (RELATIES PART-NO/COMPONENT-PART) WORDEN HIERBIJ GENEGEERD !!!
--
...
--
pl_header_part_no            varchar2(100)   := upper(p_header_part_no);
...
BEGIN
  ...
  then  
    ...
    dbms_output.put_line('MAINPART: '||pl_header_part_no ||' show bom-items J/N: '||pl_show_incl_items_jn );
    dbms_output.put_line('**************************************************************************************************************');
  end if;
    ...
    BEGIN
      ...
      open c_bom_items for SELECT bi2.LVL
				...
				(select bh.part_no
                 ...
                 --DEFINITIEVE-VERSIE die voor alternative=1/2 kan omgaan:
				 ...
				 and    bh.revision = (select max(boh1.revision) from bom_header boh1 where boh1.part_no = bh.part_no)
				 and    rownum = 1
				) 
				select LEVEL   LVL
