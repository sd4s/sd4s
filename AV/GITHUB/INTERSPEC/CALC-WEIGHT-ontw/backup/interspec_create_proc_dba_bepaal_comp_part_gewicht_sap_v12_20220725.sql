...
BEGIN
  ...
  then 
    ...
    dbms_output.put_line('COMP-PART-MAINPART: '||pl_header_part_no  );
    dbms_output.put_line('**************************************************************************************************************');
  end if;	
    BEGIN
	  --init
      ...
      loop 
		...
		then
          ...
          THEN
            l_remark := 'COMP-PART-MAINPART-HEADER-TYRE: DBA_BEPAAL_COMP_PART_GEWICHT';
            ...
            l_part_eenheid_kg := DBA_FNC_BEPAAL_HEADER_GEWICHT(p_header_part_no=>l_part_no
                                                              ...
                                                              ,p_show_incl_items_jn=>pl_show_incl_items_jn      --'N' 
                                                              );
			...
			then
              ...
              --
			ELSE
			  dbms_output.put_line(l_remark||l_mainpart||' kg:'||l_part_eenheid_kg);
		...
		--
		l_remark  := 'COMP-PART-HEADER: DBA_BEPAAL_COMP_PART_GEWICHT';
		--INSERT HULPTABEL: DBA_WEIGHT_COMPONENT_PART
		--(LET OP: IS AUTONOMOUS-TRANSACTION !! Commit zit in procedure..)
		if nvl(pl_insert_weight_comp_jn,'N') = 'J'
        then
          ...
          dbms_output.put_line(l_remark||'COMP-PART: '||l_part_no||' COMP: '||l_component_part||' kg:'||l_component_part_eenheid_kg);
  ...
  then   
    dbms_output.put_line('**************************************************************************************************************');
    dbms_output.put_line('COMP-PART EINDE BEREKEN TOTAALGEWICHT VAN HEADER: '||pl_header_part_no ||' REVISION: '||pl_header_revision||' WEIGHT: '||l_part_eenheid_kg||' COMP-PART-WEIGHT: '||l_component_part_eenheid_kg);
    dbms_output.put_line('**************************************************************************************************************');
  end if;
  --
