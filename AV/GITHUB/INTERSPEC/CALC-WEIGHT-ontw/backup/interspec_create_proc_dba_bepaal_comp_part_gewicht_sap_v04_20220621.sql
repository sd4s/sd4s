...
BEGIN
    ...
    BEGIN
      ...
      open c_bom_items for SELECT bi2.LVL
				...
				FROM   ( SELECT bi.part_no
						 ...
						 ,      bi.component_part
						 --,      (select bh2.base_quantity from bom_header bh2 where bh2.part_no = bi.part_no and bh2.revision = bi.revision and bh2.alternative= bi.alternative )   item_header_base_quantity
  ...
  then   
    dbms_output.put_line('**************************************************************************************************************');
    dbms_output.put_line('EINDE BEREKEN TOTAALGEWICHT VAN HEADER: '||pl_header_part_no ||' WEIGHT: '||l_component_part_eenheid_kg);
    dbms_output.put_line('**************************************************************************************************************');
  end if;
  --
