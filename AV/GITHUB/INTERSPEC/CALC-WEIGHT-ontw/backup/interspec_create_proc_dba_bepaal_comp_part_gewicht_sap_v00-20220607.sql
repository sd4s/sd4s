  ...
  dbms_output.put_line('**************************************************************************************************************');
    BEGIN
      ...
      dbms_output.put_line('l_mainpart'||';'||'l_mainrevision'||';'||'l_mainplant'||';''l_mainalternative'||';'||'l_mainframeid'||
		                    ';'||'l_part_no'||';'||'l_revision'||';'||'l_plant'||';''l_alternative'||';'||'l_component_part'||';'||'l_componentdescription'||';'||'l_characteristic_id'||';'||'l_functiecode'||';'||'l_path'||';'||'l_quantity_path'||';'||'l_bom_quantity_kg' );      
      ...
      open c_bom_items for SELECT bi2.LVL
                ...
                where bi2.component_part IN (select bi3.part_no from bom_item bi3 where bi3.part_no = bi2.component_part)   --NU itt bepaal-bom-gewicht NIET de selectie voor material-codes, DIE gaan we niet de gewichten berekenen...
				;
      loop 
		...
		--
		--INSERT HULPTABEL: DBA_WEIGHT_COMP_PART
