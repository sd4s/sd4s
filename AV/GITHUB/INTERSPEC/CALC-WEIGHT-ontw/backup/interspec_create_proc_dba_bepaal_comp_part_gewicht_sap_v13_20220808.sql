    ...
    BEGIN
      ...
      loop 
		...
		then
          ...
          THEN
            ...
            l_part_eenheid_kg := DBA_FNC_BEPAAL_HEADER_GEWICHT(p_header_part_no=>l_part_no
                                                              ...
                                                              ,p_show_incl_items_jn=>decode(pl_show_incl_items_jn,'U','J','J','N','N')      --ALLEEN indien U dan ook output vanuit header
                                                              );
			...
			ELSE
			  if upper(pl_show_incl_items_jn) in ('J','U')
              then  dbms_output.put_line(l_remark||'('||l_mainpart||') kg:'||l_part_eenheid_kg);
			  END IF;
			  --
        ...
        then
          ...
          if upper(pl_show_incl_items_jn) in ('J','U')
          then dbms_output.put_line(l_remark||'(COMP-PART: '||l_part_no||' COMP: '||l_component_part||') kg:'||l_component_part_eenheid_kg);
		  end if;
		  --
