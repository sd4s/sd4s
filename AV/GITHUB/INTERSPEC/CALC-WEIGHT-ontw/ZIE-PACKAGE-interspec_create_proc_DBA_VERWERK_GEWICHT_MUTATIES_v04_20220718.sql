...
create or replace procedure DBA_VERWERK_GEWICHT_MUTATIES 
DETERMINISTIC
AS
...
--
--selectie-input + sync-stuurtabel-info:
...
BEGIN
 ...
 l_str_startdatum_tm := to_char( (trunc(sysdate)-1/(24*60+60)), 'dd-mm-yyyy hh24:mi:ss' );
 --
 ...
 then
  ...
  loop
	...
	then
	  ...
	  --Het komt voor dat we wel nieuwe REVISION hebben, maar dat het voor gewichtsberekening niets uitmaakt. In dit geval
	  --nemen we gerelateerde TYRES toch gewoon me in de controle/berekening. 
	  --Pas op het moment dat we voor SAP-INTERFACE gaan bepalen voor welke TYRES/COMPONENTS de gewichten zijn gewijzigd
	  --filteren we de juiste WIJZIGINGEN er uit en leveren we die aan. 
