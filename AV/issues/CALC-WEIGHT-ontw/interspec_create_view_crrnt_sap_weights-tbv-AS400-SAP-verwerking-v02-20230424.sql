--**********************************************************************************************************
-- CREATE VIEWS: 	1.DBA_VW_CRRNT_SAP_WEIGHT            --> vervanger van GEWICHTSBEREKENING AS400 !!!!
--                                                       --> aangestuurd via INI-file LIMS_CLIENT\updweights.
--
--                  2.DBA_VW_CRRNT_MAINPART_PARTS        -->overzicht van gehele TYRE-BOM met berekende WEIGHTS tbv SAP.
--                                                       -->ALLEEN VOOR BEHEER-WERKZAAMHEDEN !!
--
--**********************************************************************************************************
--**********************************************************************************************************
...
--Vanuit DELPHI-app UPDWEIGHTS.PAS gebruikt door middel van volgend statement (let op: ATTRIBUTEN moeten zelfde naam houden):
--
--   select kmgkod,artkod, arwght from $lib.$tablename where kmgkod=:kmg
--
--REVISION
-- 1.0    INITIAL
-- 2.0    TYRE-PART Fixex doorgevoerd:
--        1)conditie mainpart=part-no eruit, want er komen VULCANIZED/GREEN-TYRES voor met zelfde PART-NO ALS MAINPART.
--        2)In subqueries van TYRES mainpart ipv part-no gebruiken
--        3)voor performance oracle-hints toegevoegd...
--        COMPONENT-PART Fixex doorgevoerd:
--        1)conditie op issued-date eruit halen. Er komt 1 component voor zonder issued-date ...
--        2)component-revision meenemen in subquery tech-calc-date  
--        3)voor performance oracle-hints toegevoegd...
-- 2.1    Round ARWGHT op 3 cijfers achter de komma !!!. Dit om minder verschillen te krijgen met AVSPECIFICATION_WEIGHT.WEIGHT !!
--
--**********************************************************************************************************
--**********************************************************************************************************
