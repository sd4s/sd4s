...
--               AV_BHR_BOM_HEADER_PART_NO
--
--*******************************************************************************************************
--view AV_BHR_BOM_TYRE_PART_NO met alle CURRENT-TYRES incl. SAP-CODE
...
and    bh.part_no    NOT IN (--Een TYRE mag zelf niet als bom-item-component-part voorkomen
							 ...
							 and   boi2.part_no        = sh2.part_no
							 and   boi2.revision       = sh2.revision
...
and   bh.part_no  IN (--BH.revision mag current/historic zijn maar wel bij een band die nog current is...
                      select sh2.part_no  
                      from status               s2
					  ,    specification_header sh2 
...
and   bh.revision   = (--BH.revision moet max-revision zijn (nu we ook op historic selecteren...)
                       select max(sh2.revision) 
                       from status               s2
					   ,    specification_header sh2
					   ,    bom_header           h2 
                       where  h2.part_no  = bh.part_no
                       and   sh2.part_no  = h2.part_no 
                       and   sh2.revision = h2.revision
                       and   sh2.status   = s2.status 
...
and  (  ( sh.frame_id   in ('A_PCR')
	 ...
	 OR ( sh.frame_id   in ('A_PCR','A_TBR' )
        --vanuit XG-hongarije ook TBR, en ook trail-banden !!
        and substr(bh.part_no,1,2) in ('GF','XG')
	    )
     )
;	 
...
--*******************************************************************************************************
--view AV_BHR_BOM_HEADER_PART_NO met alle CURRENT-HEADERS incl. SAP-CODE
--*******************************************************************************************************
...
and   bh.part_no  IN (--BH.revision mag current/historic zijn maar wel bij een band/component die nog current is...
                      select sh2.part_no  
                      from status               s2
					  ,    specification_header sh2 
                      where  sh2.part_no  = bh.part_no 
...
--*******************************************************************************************************
--*******************************************************************************************************
--VERSIE voor dd 15-07-2022:
select count(*) from av_bhr_bom_tyre_part_no;
--3758
select count(*) from av_bhr_bom_header_part_no;
--132210

--VERSIE na dd 15-07-2022:
select count(*) from av_bhr_bom_tyre_part_no;
--3791
select count(*) from av_bhr_bom_header_part_no;
--41905

--wat zijn verschillen in av_bhr_bom_header_part_no ?
select * from av_bhr_bom_header_part_no where part_no not in (
